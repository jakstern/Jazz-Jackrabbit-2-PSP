...
#	endif
	...
	PakWriter::PakWriter(StringView path, bool useHashIndex, bool useCompressedIndex, bool useRelativeOffsets,
		bool append, std::int32_t bufferSize)
			...
			_useRelativeOffsets(useRelativeOffsets)
	{
		...
		if (append) {
			_outputStream = std::make_unique<FileStream>(path, FileAccess::ReadWrite, bufferSize);
			...
			_outputStream = std::make_unique<FileStream>(path, FileAccess::Write, bufferSize);
