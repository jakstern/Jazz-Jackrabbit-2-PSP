...
namespace nCine
{
	...
	void Viewport::Visit()
	{
		...
		if (rootNode_ != nullptr) {
			ZoneScopedC(0x81A861);
#if defined(DEATH_TARGET_PSP)
			// The PSP camera is finalized in OnPostUpdate, after Viewport::Update performed its normal culling pass.
			// Refresh against this frame's camera and newly calculated rectangle before visiting drawables.
			PspDiagnosticsBeginPhase(PspDiagnosticsPhase::VisitCulling);
			UpdateCulling(rootNode_);
			PspDiagnosticsEndPhase(PspDiagnosticsPhase::VisitCulling);
			PspDiagnosticsBeginPhase(PspDiagnosticsPhase::VisitCollect);
#endif
			std::uint32_t visitOrderIndex = 0;
			rootNode_->OnVisit(renderQueue_, visitOrderIndex);
#if defined(DEATH_TARGET_PSP)
			PspDiagnosticsEndPhase(PspDiagnosticsPhase::VisitCollect);
#endif
		}
...
#if defined(DEATH_TARGET_PSP)
		// On PSP the screen viewport starts the GU frame and clears; the GL clear below is a null-GL no-op.
		if (type_ == Type::Screen) {
			void PspGuBeginFrame(std::uint32_t);
			const float* c = clearColor_.Data();
			const std::uint32_t clear = (std::uint32_t(c[3] * 255.0f) << 24) | (std::uint32_t(c[2] * 255.0f) << 16) |
				(std::uint32_t(c[1] * 255.0f) << 8) | std::uint32_t(c[0] * 255.0f);
			PspGuBeginFrame(clear);
		}
#endif
