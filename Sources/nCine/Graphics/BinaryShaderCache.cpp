...
namespace nCine
{
		...
		: isAvailable_(false), platformHash_(0)
	{
		...
		// Unreachable on PSP (no GL, isSupported is always false above). Guarded because the fake-GL
		// shim makes glProgramBinary a function template, which cannot bind to a function pointer.
#elif defined(WITH_OPENGLES) && !defined(DEATH_TARGET_EMSCRIPTEN) && !defined(DEATH_TARGET_SWITCH) && !defined(DEATH_TARGET_UNIX) && (!defined(DEATH_TARGET_WINDOWS_RT) || defined(WITH_ANGLE))
