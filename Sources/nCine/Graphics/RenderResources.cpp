	...
	void RenderResources::Create()
	{
		...
		for (std::uint32_t i = 0; i < std::uint32_t(arraySize(shadersToLoad)); i++) {
			...
			if (compileTwice) {
				...
				// GetSize()==0 means no real reflected block (e.g. the PSP fake-GL dummy block); skip the batch-size
				// probe rather than dividing by zero (MIPS integer divide-by-zero traps with a `break`). The shader
				// stays linked from the Introspection::Enabled pass above, which is all PSP's fake-GL path needs.
				if (block != nullptr && block->GetSize() > 0) {
