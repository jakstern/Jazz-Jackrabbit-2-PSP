...
namespace nCine
{
	...
	void Material::SetShaderProgram(GLShaderProgram* program)
	{
		...
		// PSP's fake-GL shim never links real programs (ContentResolver hands back null shaders), and the GU emitter
		// draws from the render command's geometry/textures directly, ignoring the shader. Keep the null program and
		// skip the uniform/attribute setup below, which would dereference it (`*shaderProgram_`, offset 0xc).
		// Downstream shaderProgram_ dereferences are individually guarded; RenderCommand::Issue returns before the
		// GL draw path on PSP so Material::Bind() is never reached.
	...
	{
#if defined(DEATH_TARGET_PSP)
		// ContentResolver hands back a null Shader on PSP (fake-GL precompiled shaders aren't registered). Resolve
		// null-safely so this doesn't dereference `shader` at offset 0xc; the resulting null program is stored and
		// null->null comparisons then report "no change" (avoiding per-frame shader churn on the tilemap mesh).
		GLShaderProgram* shaderProgram = (shader != nullptr ? shader->glShaderProgram_.get() : nullptr);
#else
...
#if defined(DEATH_TARGET_PSP)
		// A material may reach here with no shader on PSP (fake-GL shaders are absent); nothing to reserve.
		if (shaderProgram_ == nullptr) {
			return;
		}
#endif
	...
	std::uint32_t Material::GetSortKey()
	{
		...
		// shaderProgram_ is null on PSP (fake-GL shaders are absent); hash 0 for it instead of dereferencing null.
		hashData.shaderProgram = (shaderProgram_ != nullptr ? shaderProgram_->GetGLHandle() : 0);
