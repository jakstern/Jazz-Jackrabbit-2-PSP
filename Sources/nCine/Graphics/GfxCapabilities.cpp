...
namespace nCine
{
	...
	void GfxCapabilities::Init()
	{
		...
		// PSP has no OpenGL; drawing goes through the GU backend. Report fixed, GU-appropriate limits.
		glMajorVersion_ = 2;
		glMinorVersion_ = 0;
		glReleaseVersion_ = 0;
		glInfoStrings_.vendor = "Sony";
		glInfoStrings_.renderer = "PSP GU";
		glInfoStrings_.glVersion = "N/A";
		glInfoStrings_.glslVersion = "N/A";
		glIntValues_[(std::int32_t)GLIntValues::MAX_TEXTURE_SIZE] = 512;
		glIntValues_[(std::int32_t)GLIntValues::MAX_TEXTURE_IMAGE_UNITS] = 8;
		glIntValues_[(std::int32_t)GLIntValues::MAX_UNIFORM_BLOCK_SIZE] = 16 * 1024;
		glIntValues_[(std::int32_t)GLIntValues::MAX_UNIFORM_BLOCK_SIZE_NORMALIZED] = 16 * 1024;
		glIntValues_[(std::int32_t)GLIntValues::MAX_UNIFORM_BUFFER_BINDINGS] = 8;
		glIntValues_[(std::int32_t)GLIntValues::MAX_VERTEX_UNIFORM_BLOCKS] = 8;
		glIntValues_[(std::int32_t)GLIntValues::MAX_FRAGMENT_UNIFORM_BLOCKS] = 8;
		glIntValues_[(std::int32_t)GLIntValues::UNIFORM_BUFFER_OFFSET_ALIGNMENT] = 256;
		glIntValues_[(std::int32_t)GLIntValues::MAX_VERTEX_ATTRIB_STRIDE] = 2048;
		glIntValues_[(std::int32_t)GLIntValues::MAX_COLOR_ATTACHMENTS] = 1;
		return;
#else
...
#	endif
		...
		LOGI("--- OpenGL extensions ---");*/
#endif
#endif // !DEATH_TARGET_PSP
	}

	void GfxCapabilities::CheckGLExtensions(const char* extensionNames[], bool results[], std::uint32_t numExtensionsToCheck) const
	{
#if defined(DEATH_TARGET_PSP)
		// Never called on PSP (Init() returns early); no GL extensions exist
		(void)extensionNames; (void)results; (void)numExtensionsToCheck;
#else
		GLint numExtensions;
		glGetIntegerv(GL_NUM_EXTENSIONS, &numExtensions);
		if (numExtensions <= 0) {
