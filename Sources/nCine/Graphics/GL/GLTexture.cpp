...
namespace nCine
{
	...
	GLTexture::~GLTexture()
	{
		...
		// The PSP emitter owns CPU/GU-visible pixel storage keyed by this object.
		// glDeleteTextures() is only a shim on PSP and cannot release that storage;
		// without this hook every level/menu transition leaked all uploaded textures.
		void PspReleaseTexture(const GLTexture*);
		PspReleaseTexture(this);
#endif
...
#if defined(DEATH_TARGET_PSP)
		if (level == 0) {
			void PspCaptureTexture(const GLTexture*, int, int, unsigned int, unsigned int, const void*);
			PspCaptureTexture(this, width, height, format, type, data);
		}
#endif
	}
...
#if defined(DEATH_TARGET_PSP)
		if (level == 0) {
			void PspUpdateSubTexture(const GLTexture*, int, int, int, int, const void*);
			PspUpdateSubTexture(this, xoffset, yoffset, width, height, data);
		}
#endif
	}
