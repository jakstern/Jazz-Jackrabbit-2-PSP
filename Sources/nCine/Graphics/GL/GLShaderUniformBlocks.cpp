...
namespace nCine
{
	...
	GLUniformBlockCache* GLShaderUniformBlocks::GetUniformBlock(const char* name)
	{
		...
		// No GL uniform-block reflection on PSP, so blocks are never found. Return a shared empty dummy instead of
		// null: the many callers that do `UniformBlock(...)->GetUniform(...)->SetX()` or bind `*UniformBlock(...)`
		// would otherwise fault on a null `this` (masked-but-slow reads all over the render path). The dummy's
		// GetUniform() returns the no-op dummy uniform cache (all Set*/Get* are null-safe). Callers that instead do
		// size math on the block (RenderResources::Create) must guard GetSize()==0 - see that call site.
		if (uniformBlockCache == nullptr) {
			static GLUniformBlockCache dummy;
			return &dummy;
		}
#endif
