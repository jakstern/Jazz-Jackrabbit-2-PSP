...
namespace nCine
{
	...
	GLUniformCache* GLShaderUniforms::GetUniform(const char* name)
	{
		...
		// No GL uniform reflection on PSP - return a shared no-op dummy so callers that don't null-check
		// `Uniform(...)->Set...()` don't dereference null and crash (its Set*/Get* are safe no-ops).
		if (uniformCache == nullptr) {
