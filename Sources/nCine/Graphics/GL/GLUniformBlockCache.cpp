...
namespace nCine
{
	...
	{
		GLUniformCache* cache = uniformCaches_.find(String::nullTerminatedView(name));
#if defined(DEATH_TARGET_PSP)
		// On PSP there is no GL uniform reflection, so instance-block uniforms are absent and callers that don't
		// null-check `GetUniform(...)->Set...()` would dereference null and crash. Return a shared dummy cache
		// instead: its uniform_/dataPointer_ are null, so every Get*/Set* is a safe no-op (the GU emitter reads
		// the values it needs from the RenderCommand directly, not from these uniforms).
		if (cache == nullptr) {
			static GLUniformCache dummy;
			return &dummy;
		}
#endif
		return cache;
