...
#	include "Backends/Psp/PspDiagnostics.h"
#	include "Backends/Psp/PspGu.h"
#	include "Backends/Psp/PspHardware.h"
#endif
...
namespace nCine
{
	...
	void Application::InitCommon()
	{
		...
		// The PSP audio device is registered by the PSP backend (see PspBackend.cpp).
#if defined(WITH_THREADS)
	...
	{
#if defined(DEATH_TARGET_PSP)
		PspDiagnosticsBeginFrame();
#endif
		frameTimer_->AddFrame();

		// The original 32 MiB PSP needs a 30 Hz presentation budget, but Jazz2 collision and actor interaction
		// assume the nominal 60 Hz simulation cadence. Run every update and submit every second frame on 01g;
		// later models retain the normal one-update/one-render path.
		bool renderThisFrame = true;
#if defined(DEATH_TARGET_PSP)
		renderThisFrame = !PspIsLowMemoryModel() || ((frameTimer_->GetTotalNumberFrames() & 1u) == 0u);
#endif
...
#if defined(DEATH_TARGET_PSP)
			PspDiagnosticsBeginPhase(PspDiagnosticsPhase::Begin);
#endif
#if defined(NCINE_PROFILING)
			profileStartTime_ = TimeStamp::now();
#endif
...
#if defined(DEATH_TARGET_PSP)
			PspDiagnosticsEndPhase(PspDiagnosticsPhase::Begin);
#endif
#if defined(NCINE_PROFILING)
		...
		if (appCfg_.withScenegraph) {
...
#if defined(DEATH_TARGET_PSP)
				PspDiagnosticsBeginPhase(PspDiagnosticsPhase::Update);
#endif
#if defined(NCINE_PROFILING)
				profileStartTime_ = TimeStamp::now();
#endif
...
#if defined(DEATH_TARGET_PSP)
				PspDiagnosticsEndPhase(PspDiagnosticsPhase::Update);
#endif
#if defined(NCINE_PROFILING)
...
#if defined(DEATH_TARGET_PSP)
				PspDiagnosticsBeginPhase(PspDiagnosticsPhase::PostUpdate);
#endif
#if defined(NCINE_PROFILING)
				profileStartTime_ = TimeStamp::now();
#endif
...
#if defined(DEATH_TARGET_PSP)
				PspDiagnosticsEndPhase(PspDiagnosticsPhase::PostUpdate);
#endif
#if defined(NCINE_PROFILING)
			...
			if (appCfg_.withGraphics && renderThisFrame) {
				{
					ZoneScopedNC("Visit", 0x81A861);
#if defined(DEATH_TARGET_PSP)
					PspDiagnosticsBeginPhase(PspDiagnosticsPhase::Visit);
#endif
#if defined(NCINE_PROFILING)
					profileStartTime_ = TimeStamp::now();
#endif
...
#if defined(DEATH_TARGET_PSP)
					PspDiagnosticsEndPhase(PspDiagnosticsPhase::Visit);
#endif
#if defined(NCINE_PROFILING)
...
#endif
#if defined(DEATH_TARGET_PSP)
					PspDiagnosticsBeginPhase(PspDiagnosticsPhase::Queue);
#endif
					screenViewport_->SortAndCommitQueue();
#if defined(DEATH_TARGET_PSP)
					PspDiagnosticsEndPhase(PspDiagnosticsPhase::Queue);
					// Frame N has already been submitted. Update/visit/queue for N+1 ran while the GE consumed it;
					// fence and present only now, immediately before the direct list and draw buffer are reused.
					PspGuPrepareFrame();
					PspDiagnosticsBeginPhase(PspDiagnosticsPhase::Emit);
#endif
					screenViewport_->Draw();
#if defined(DEATH_TARGET_PSP)
					PspDiagnosticsEndPhase(PspDiagnosticsPhase::Emit);
#endif
#if defined(NCINE_PROFILING)
