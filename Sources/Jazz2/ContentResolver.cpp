...
namespace nCine { void PspSetPendingPackTexture(std::uint64_t nameHash, bool streamed = false); }
...
namespace Jazz2
{
	...
	void ContentResolver::Release()
	{
		...
		_cachedMetadata.rehash(0);
		_cachedGraphics.clear();
		_cachedGraphics.rehash(0);
#if defined(WITH_AUDIO)
		_cachedSounds.clear();
		_cachedSounds.rehash(0);
#endif
		...
		// Release the GPU-side palette allocation too; it is recreated lazily on the next indexed draw.
...
#	endif
...
#if !defined(DEATH_TARGET_EMSCRIPTEN)
	void ContentResolver::UnmountPaks()
	{
		_mountedPaks.clear();
	}
	...
	void ContentResolver::RemountPaks()
	{
		...
		for (auto item : fs::Directory(GetContentPath(), fs::EnumerationOptions::SkipDirectories)) {
			...
			if (extension != "pak"_s || fs::GetFileName(item) == "texture.pak"_s) {
				continue; // texture.pak is the PSP GU texture pack (loaded by PspAssetPack), not a PakFile archive
			}
...
#	endif
	...
	void ContentResolver::GetPspMemoryFootprint(std::uint32_t& maskBytes, std::uint16_t& metadataCount,
		std::uint16_t& graphicsCount, std::uint16_t& soundsCount) const
	{
		std::uint32_t masks = 0;
		std::uint16_t graphics = 0;
		for (const auto& [key, gfx] : _cachedGraphics) {
			graphics++;
			if (gfx->Mask != nullptr) {
				const std::size_t texels = (std::size_t)gfx->FrameConfiguration.X * gfx->FrameDimensions.X
					* (std::size_t)gfx->FrameConfiguration.Y * gfx->FrameDimensions.Y;
				masks += (std::uint32_t)PspGpu::MaskPackedBytes(texels);
			}
		}
		maskBytes = masks;
		graphicsCount = graphics;
		metadataCount = (std::uint16_t)_cachedMetadata.size();
#	if defined(WITH_AUDIO)
		soundsCount = (std::uint16_t)_cachedSounds.size();
#	else
		soundsCount = 0;
#	endif
	}
	...
	Metadata* ContentResolver::RequestMetadata(StringView path, bool forceIndexed, bool forceStreamed)
	{
		auto pathNormalized = fs::ToNativeSeparators(path);
		// Indexed and streamed metadata variants need separate entries: the linked graphic resources carry those
		// properties, so allowing whichever variant loaded first to win would make later requests silently resident
		// or non-recolorable. "*" and "~" cannot appear in a real metadata path.
		String cacheKey = (forceIndexed
			? (forceStreamed ? String(pathNormalized + "*~"_s) : String(pathNormalized + "*"_s))
			: (forceStreamed ? String(pathNormalized + "~"_s) : String(pathNormalized)));
		...
		auto bytesRead = s->Read(buffer.get(), fileSize);
		...
		bool parsedOk = reader->parse(buffer.get(), buffer.get() + fileSize, &doc, &errors);
		if (parsedOk) {
			...
			if (animations.isObject()) {
				...
				for (auto it = animations.begin(); it != animations.end(); ++it) {
					...
					bool streamedByMetadata = false;
					(*it)["Streamed"].get(streamedByMetadata);
					bool streamed = forceStreamed || streamedByMetadata;
					graphics.Base = RequestGraphics(assetPath, (std::uint16_t)paletteOffset, keepIndexed, streamed);
		...
		bool forceStreamed)
	{
		...
		auto pathNormalized = fs::ToNativeSeparators(path);
		// Player sprite sheets already use streamed residency on PSP; include that effective behavior in the key too,
		// so an explicit streamed request cannot create a duplicate resource for the same path.
		forceStreamed = forceStreamed || pathNormalized.hasPrefix("Jazz/"_s) || pathNormalized.hasPrefix("Spaz/"_s) ||
			pathNormalized.hasPrefix("Lori/"_s);
		std::uint32_t cacheKeyOffset = GetGraphicsCacheKey(paletteOffset, keepIndexed, forceStreamed);
		...
		if (fs::GetExtension(pathNormalized) == "aura"_s) {
			return RequestGraphicsAura(pathNormalized, paletteOffset, keepIndexed, forceStreamed);
		}
		...
		if (reader->parse(buffer.get(), buffer.get() + fileSize, &doc, &errors)) {
			...
			if (texLoader->hasLoaded()) {
				...
				if (needsMask) {
					// 1 bit/texel coverage mask (make_unique value-initializes to zero). See PspGpu::MaskAlphaThreshold.
					graphics->Mask = std::make_unique<std::uint8_t[]>(PspGpu::MaskPackedBytes((std::size_t)w * h));
					for (std::int32_t i = 0; i < w * h; i++) {
						if (pixels[(i * PixelSize) + 3] > PspGpu::MaskAlphaThreshold) graphics->Mask[i >> 3] |= (std::uint8_t)(1u << (i & 7));
					}
				}
				if (palette != nullptr) {
	...
	GenericGraphicResource* ContentResolver::RequestGraphicsAura(StringView path, std::uint16_t paletteOffset,
		bool keepIndexed, bool forceStreamed)
	{
		// PSP: read the .aura header from the baked pack (no file open) when this sprite is baked; the pack
		// branch below then builds the texture + mask from the pack too. Only fall back to the on-disk .aura
		// (the runtime decode path) when a sprite isn't in the pack.
		std::uint8_t packHdr[64];
		std::unique_ptr<Stream> s;
		{
			PspAssetPack::Get().EnsureLoaded(String(GetCachePath()).data());
			const PspGpu::TexEntry* pe = PspAssetPack::Get().Find(PspGpu::NameHash(String(path).data()));
			if (pe != nullptr && PspAssetPack::Get().ReadMeta(*pe, packHdr, sizeof(packHdr))) {
				s = std::make_unique<MemoryStream>((const void*)packHdr, (std::int64_t)pe->metaSize);
			}
		}
		if (s == nullptr) {
			s = OpenContentFile(fs::CombinePath("Animations"_s, path));
		}
		...
		// PSP: this sprite is pre-baked in the GU pack. Build the resource from the .aura header we just read
		// plus the pack (texture + collision mask), skipping the QOI decode / palette / atlas entirely. The
		// runtime path below is the fallback for anything not baked (e.g. 32-bit tilesets' extras).
		{
			PspAssetPack::Get().EnsureLoaded(String(GetCachePath()).data());
			const std::uint64_t packKey = PspGpu::NameHash(String(path).data());
			const PspGpu::TexEntry* packE = PspAssetPack::Get().Find(packKey);
			if (packE != nullptr) {
				auto packGfx = std::make_unique<GenericGraphicResource>();
				packGfx->Flags |= GenericGraphicResourceFlags::Referenced;
				if (packE->format != (std::uint8_t)PspGpu::PixelFormat::Rgba8888) {
					packGfx->Flags |= GenericGraphicResourceFlags::Indexed; // recolored at draw time
				}
				if ((flags & 0x02) != 0x02) { // needsMask
					const std::size_t maskBytes = PspGpu::MaskPackedBytes((std::size_t)width * height);
					packGfx->Mask = std::make_unique<std::uint8_t[]>(maskBytes);
					if (!PspAssetPack::Get().ReadMask(*packE, packGfx->Mask.get(), maskBytes)) {
						packGfx->Mask = nullptr;
					}
				}
				Texture::Format fmt = (packE->format == (std::uint8_t)PspGpu::PixelFormat::Indexed8 ? Texture::Format::R8
					: (packE->format == (std::uint8_t)PspGpu::PixelFormat::Rg8 ? Texture::Format::RG8 : Texture::Format::RGBA8));
				nCine::PspSetPendingPackTexture(packKey, forceStreamed);
				packGfx->TextureDiffuse = std::make_unique<Texture>(path.data(), fmt, (std::int32_t)width, (std::int32_t)height);
				packGfx->TextureDiffuse->SetMinFiltering(SamplerFilter::Nearest);
				packGfx->TextureDiffuse->SetMagFiltering(SamplerFilter::Nearest);

				packGfx->AnimDuration = animDuration / 256.0f;
				packGfx->FrameDimensions = Vector2i(frameDimensionsX, frameDimensionsY);
				packGfx->FrameConfiguration = Vector2i(frameConfigurationX, frameConfigurationY);
				packGfx->FrameCount = frameCount;
				packGfx->Hotspot = (hotspotX != UINT16_MAX || hotspotY != UINT16_MAX) ? Vector2i(hotspotX, hotspotY) : Vector2i();
				packGfx->Coldspot = (coldspotX != UINT16_MAX || coldspotY != UINT16_MAX) ? Vector2i(coldspotX, coldspotY) : Vector2i(InvalidValue, InvalidValue);
				packGfx->Gunspot = (gunspotX != UINT16_MAX || gunspotY != UINT16_MAX) ? Vector2i(gunspotX, gunspotY) : Vector2i(InvalidValue, InvalidValue);

				std::uint32_t packCacheKey = GetGraphicsCacheKey(paletteOffset, keepIndexed, forceStreamed);
				return _cachedGraphics.emplace(Pair(String(path), packCacheKey), std::move(packGfx)).first->second.get();
			}
		}
		...
		if (needsMask) {
			// 1 bit/texel coverage mask (make_unique value-initializes to zero). The decoded buffer is tightly packed
			// to `channelCount` bytes/pixel: a 1-channel (index-only) sprite is opaque except index 0 (transparent),
			// a 2-channel sprite has explicit alpha in green, anything wider keeps alpha in the 4th byte. Bit set when
			// coverage exceeds PspGpu::MaskAlphaThreshold (matches the pre-bit-packing runtime compare).
			graphics->Mask = std::make_unique<std::uint8_t[]>(PspGpu::MaskPackedBytes((std::size_t)width * height));
			for (std::uint32_t i = 0; i < width * height; i++) {
				std::uint8_t coverage;
				if (channelCount == 1) {
					coverage = (pixels[i] != 0 ? 255 : 0);
				} else if (channelCount == 2) {
					coverage = pixels[(i * 2) + 1];
				} else {
					coverage = pixels[(i * channelCount) + 3];
				}
				if (coverage > PspGpu::MaskAlphaThreshold) graphics->Mask[i >> 3] |= (std::uint8_t)(1u << (i & 7));
			}
		}
		if (palette != nullptr) {
			for (std::uint32_t i = 0; i < width * height; i++) {
	...
	void ContentResolver::ReadImageFromFile(std::unique_ptr<Stream>& s, std::uint8_t* data, std::int32_t width, std::int32_t height, std::int32_t channelCount)
	{
		...
		for (std::int32_t px_pos = 0; px_pos < px_len; px_pos += channelCount) {
			...
			// `data` is packed by channel count. Indexed and RG pixels are not 32-bit aligned after the first
			// pixel, so storing the union directly faults on real MIPS hardware. Copy only the active channels.
	...
	std::unique_ptr<Tiles::TileSet> ContentResolver::RequestTileSet(StringView path, std::uint16_t captionTileId, bool applyPalette, const std::uint8_t* paletteRemapping)
	{
		...
		if (fullPath.empty()) {
			...
			if (!fs::IsReadableFile(fullPath)) {
				// PSP cache sidecars deliberately omit diffuse QOI; graphics are mandatory texture.pak entries.
				fullPath = fs::CombinePath({ GetCachePath(), "Tilesets"_s, String(path + ".j2tpsp"_s) });
			}
		}
		...
		// Per-tile opaque flag (hidden-debris cull) and caption thumbnail aren't produced on PSP - the atlas is
		// pre-baked on the host, so the raw QOI image is never decoded here.
		std::unique_ptr<std::uint8_t[]> tileDiffuseOpaque;
		bool indexTiles = true; // PSP tilesets are baked as indexed (GU_PSM_T8)
		(void)width; (void)height; (void)paletteRemapping; (void)captionTileId;

		// PSP: the tileset atlas is pre-baked into the GU pack (Tools/psp-bake -> Cache/PspTextures.pack) as
		// swizzled/paged T8 + CLUT. Look it up by tileset name and create a placeholder Texture whose GU capture
		// is served straight from the pack - no on-device QOI decode or atlas assembly. The raw image that follows
		// the compressed block is left unread. No fallback: the pack is a required part of the PSP data.
		{
			PspAssetPack& pack = PspAssetPack::Get();
			pack.EnsureLoaded(String(GetCachePath()).data());
			const std::uint64_t key = PspGpu::NameHash(String(path).data());
			const PspGpu::TexEntry* e = pack.Find(key);
			if (e != nullptr) {
				nCine::PspSetPendingPackTexture(key);
				textureDiffuse = std::make_unique<Texture>(fullPath.data(), Texture::Format::R8,
					(std::int32_t)e->width, (std::int32_t)e->height);
			} else {
				LOGW("No baked PSP texture for tileset \"{}\" (run psp-bake)", path);
			}
		}
...
#if defined(WITH_AUDIO)
#if defined(DEATH_TARGET_PSP)
		// psp-bake converts tracker/J2B sources to streamable IMA ADPCM WAVs.
		// Preserve the level's base filename but replace its source extension.
		String musicName(fs::GetFileNameWithoutExtension(path));
		musicName += ".wav"_s;
		String fullPath = fs::CombinePath({ GetCachePath(), "Music"_s, musicName });
		if (!fs::IsReadableFile(fullPath)) return nullptr;
		auto player = std::make_unique<AudioStreamPlayer>(fullPath);
		return (player->frequency() > 0 ? std::move(player) : nullptr);
#else
		// Don't load sounds in headless mode
		if (_isHeadless) {
			return nullptr;
	...
	void ContentResolver::RecreateGemPalettes()
	{
		...
		// Rebuild the 180-degree hue-rotated copy of the base palette (row 3). Sprites drawn through this row's
		// paletteOffset render as their opposite hue with no second texture (the Electro bullet reuses the blue
		// Devan energy-bolt sprite this way). Derived from the live base palette, so it tracks per-level changes.
		std::uint32_t* hueRow = _palettes + Hue180PaletteRow * ColorsPerPalette;
		for (std::int32_t i = 0; i < ColorsPerPalette; i++) {
			hueRow[i] = ShiftHue(_palettes[i], 180.0f);
		}

		// The base palette (row 0), the gem rows and the hue-rotated row just changed - mark for re-upload.
		MarkPaletteDirty(0, Hue180PaletteRow);
