...
#if defined(DEATH_TARGET_PSP) && defined(JAZZ2_PSP_DEBUG)
#	include "../nCine/Backends/Psp/PspDiagnostics.h"
#	define PSP_STAGE(msg) ::nCine::PspDiagnosticsLog(msg)
#else
#	define PSP_STAGE(msg) ((void)0)
#endif
...
namespace Jazz2
{
	...
	LevelHandler::LevelHandler(IRootController* root)
		: _root(root), _lightingShader(nullptr), _blurShader(nullptr), _downsampleShader(nullptr), _combineShader(nullptr),
			...
			_cheatsUsed(false),
#if defined(DEATH_TARGET_PSP)
			_pspSimulationPaused(false),
#endif
			_checkpointCreated(false), _nextLevelType(ExitType::None),
	...
	bool LevelHandler::Initialize(const LevelInitialization& levelInit)
	{
		...
		PSP_STAGE("level: Initialize enter");
		...
		PSP_STAGE("level: TryLoadLevel enter");
		...
		PSP_STAGE("level: TryLoadLevel done");

		AttachComponents(std::move(descriptor));
		PSP_STAGE("level: AttachComponents done");
		SpawnPlayers(levelInit);
		PSP_STAGE("level: SpawnPlayers done");

		OnInitialized();
		PSP_STAGE("level: OnInitialized done");
		resolver.EndLoading();
		PSP_STAGE("level: EndLoading done");
	...
	void LevelHandler::AttachComponents(LevelDescriptor&& descriptor)
	{
		...
		PSP_STAGE("level: AttachComponents tilemap");
		...
		if (!_musicCurrentPath.empty()) {
			...
			if (_music != nullptr) {
				...
				// PSP presents a dedicated loading frame while the level is built. Starting
				// here lets the music escape before the first gameplay frame reaches the
				// display; the PSP event handler starts it after that first present instead.
#if !defined(DEATH_TARGET_PSP)
				_music->play();
#endif
			}
	...
	void LevelHandler::OnBeginFrame()
	{
...
#if defined(DEATH_TARGET_PSP)
		// No pause menu / console on PSP - just refresh the input state each frame.
		{
			UpdatePressedActions();
		}

		if (_pspFlyCheat && !_players.empty() &&
			_players[0]->GetModifier() != Actors::Player::Modifier::Airboard) {
			OnConsoleCommand("jjfly"_s);
		}
		...
		if (_pspMenuActive) {
			...
			input.RequiredMovement = Vector2f::Zero;
		} else {
			constexpr std::uint32_t MenuButtons = PSP_CTRL_CROSS | PSP_CTRL_CIRCLE | PSP_CTRL_SQUARE |
				PSP_CTRL_TRIANGLE | PSP_CTRL_LTRIGGER | PSP_CTRL_RTRIGGER | PSP_CTRL_START;
			if (_pspSuppressInputUntilRelease && (pad.Buttons & MenuButtons) != 0) {
				input.PressedActions = 0;
				input.RequiredMovement = Vector2f::Zero;
			} else {
				_pspSuppressInputUntilRelease = false;
				std::uint64_t actions = 0;
				float mx = 0.0f, my = 0.0f;
				auto set = [&](PlayerAction action) { actions |= (1ull << std::int32_t(action)); };
				if (pad.Buttons & PSP_CTRL_LEFT) { set(PlayerAction::Left); mx = -1.0f; }
				if (pad.Buttons & PSP_CTRL_RIGHT) { set(PlayerAction::Right); mx = 1.0f; }
				if (pad.Buttons & PSP_CTRL_UP) { set(PlayerAction::Up); my = -1.0f; }
				if (pad.Buttons & PSP_CTRL_DOWN) { set(PlayerAction::Down); set(PlayerAction::Buttstomp); my = 1.0f; }
				if (pad.Buttons & PSP_CTRL_CROSS) set(PlayerAction::Jump);
				if (pad.Buttons & (PSP_CTRL_SQUARE | PSP_CTRL_RTRIGGER)) set(PlayerAction::Fire);
				if (pad.Buttons & (PSP_CTRL_CIRCLE | PSP_CTRL_LTRIGGER)) set(PlayerAction::Run);
				if (pad.Buttons & PSP_CTRL_TRIANGLE) set(PlayerAction::ChangeWeapon);
				if (!(pad.Lx == 0 && pad.Ly == 0)) {
					constexpr int AnalogCenter = 128;
					constexpr int AnalogDeadzone = 32;
					// Down is deliberately harder to trigger than the other directions: it must be pushed well past
					// the deadzone AND be more down than sideways. A shallow "right + slight down" used to register
					// Down, which crouches and blocks the jump when the player only meant to run and jump.
					constexpr int DownActivate = 52;
					constexpr int ButtstompMinDown = 56;
					constexpr float AnalogMaxMagnitude = 127.0f;
					constexpr float AnalogCurveExponent = 1.25f;
					const int dx = (int)pad.Lx - AnalogCenter;
					const int dy = (int)pad.Ly - AnalogCenter;
					float magnitude = std::sqrt((float)(dx * dx + dy * dy));
					if (magnitude > AnalogDeadzone) {
						// Rescale the usable radial range to 0..1, then apply a mild curve for finer slow walking.
						float normalizedMagnitude = std::min((magnitude - AnalogDeadzone) /
							(AnalogMaxMagnitude - AnalogDeadzone), 1.0f);
						float curvedMagnitude = std::pow(normalizedMagnitude, AnalogCurveExponent);
						float analogScale = curvedMagnitude / magnitude;
						float analogX = dx * analogScale;
						float analogY = dy * analogScale;
						if (analogX < 0.0f) { set(PlayerAction::Left); mx = analogX; }
						else if (analogX > 0.0f) { set(PlayerAction::Right); mx = analogX; }
						if (dy < -AnalogDeadzone) { set(PlayerAction::Up); my = analogY; }
						else if (dy > DownActivate && dy >= std::abs(dx)) {
							// Deliberate, steep-enough down (within 45 degrees of straight down). Shallow diagonals
							// fall through here and never crouch.
							set(PlayerAction::Down);
							my = analogY;
							// Buttstomp is narrower still: within about 27 degrees of straight down, so downward
							// diagonals move down without attacking.
							if (dy >= ButtstompMinDown && std::abs(dx) * 2 <= dy) set(PlayerAction::Buttstomp);
						}
					}
				}
				input.PressedActions = actions;
				input.RequiredMovement = Vector2f(mx, my);
			}
		}
#else
...
#	endif
	...
	void LevelHandler::PauseForPspMenu() { _pspMenuActive = true; PauseGame(); }
	void LevelHandler::ResumeFromPspMenu() { _pspMenuActive = false; _pspSuppressInputUntilRelease = true; ResumeGame(); }
	void LevelHandler::SetPspMenuBackgroundSimulation(bool enabled)
	{
		if (enabled) ResumeGame();
		else PauseGame();
	}
	void LevelHandler::SuppressInputUntilRelease() { _pspSuppressInputUntilRelease = true; }
	bool LevelHandler::IsPspNormalLevelExitActive() const
	{
		return _nextLevelType != ExitType::None &&
			(_nextLevelType & ExitType::FastTransition) != ExitType::FastTransition &&
			(_nextLevelType & ExitType::TypeMask) == ExitType::Normal;
	}
	void LevelHandler::StartPspMusicAfterFirstPresent()
	{
#if defined(WITH_AUDIO)
		if (_music != nullptr) _music->play();
#endif
	}
	void LevelHandler::ApplyPspMusicVolume()
	{
#if defined(WITH_AUDIO)
		const float gain = PreferencesCache::MasterVolume * PreferencesCache::MusicVolume;
		if (_music != nullptr) _music->setGain(gain);
		if (_sugarRushMusic != nullptr) _sugarRushMusic->setGain(gain);
#endif
	}
	bool LevelHandler::HasPspLevelTransition() const { return _nextLevelType != ExitType::None; }
	void LevelHandler::PspCheatSkipForward()
	{
		_cheatsUsed = true;
		const bool allowed = PreferencesCache::AllowCheats;
		PreferencesCache::AllowCheats = true;
		OnConsoleCommand("jjnext"_s);
		PreferencesCache::AllowCheats = allowed;
	}
	void LevelHandler::PspCheatSkipTo(StringView levelName)
	{
		_cheatsUsed = true;
		BeginLevelChange(nullptr, ExitType::Warp | ExitType::FastTransition, levelName);
	}
	bool LevelHandler::PspInvokeCheat(StringView command)
	{
		const bool allowed = PreferencesCache::AllowCheats;
		PreferencesCache::AllowCheats = true;
		const bool handled = OnConsoleCommand(command);
		PreferencesCache::AllowCheats = allowed;
		return handled;
	}
	ShieldType LevelHandler::PspCheatCycleShield()
	{
		if (_players.empty()) return ShieldType::None;

		ShieldType next;
		switch (_players[0]->GetActiveShield()) {
			case ShieldType::None: next = ShieldType::Water; break;
			case ShieldType::Water: next = ShieldType::Fire; break;
			case ShieldType::Fire: next = ShieldType::Lightning; break;
			default: next = ShieldType::None; break;
		}
		for (auto* player : _players) {
			player->SetShield(next, next != ShieldType::None ? 40.0f * FrameTimer::FramesPerSecond : 0.0f);
		}
		_cheatsUsed = true;
		return next;
	}
	void LevelHandler::SetPspFlyCheat(bool enabled)
	{
		if (_pspFlyCheat == enabled) return;
		_pspFlyCheat = enabled;
		const bool allowed = PreferencesCache::AllowCheats;
		PreferencesCache::AllowCheats = true;
		OnConsoleCommand("jjfly"_s);
		PreferencesCache::AllowCheats = allowed;
		if (enabled) _cheatsUsed = true;
	}
	void LevelHandler::SetPspGodCheat(bool enabled)
	{
		if (_pspGodCheat == enabled) return;
		_pspGodCheat = enabled;
		if (enabled) {
			const bool allowed = PreferencesCache::AllowCheats;
			PreferencesCache::AllowCheats = true;
			OnConsoleCommand("jjgod"_s);
			PreferencesCache::AllowCheats = allowed;
		} else {
			for (auto* player : _players) player->SetInvulnerability(0.0f, Actors::Player::InvulnerableType::Shielded);
		}
		if (enabled) _cheatsUsed = true;
	}
...
#if defined(DEATH_TARGET_PSP)
		// Keep TileMap/viewport end-of-frame recycling outside this gate, but do not
		// advance collisions, cameras, audio positions, or elapsed time while the PSP
		// pause/completion presentation has frozen the level.
		if (!_pspSimulationPaused) {
#else
		if (!IsPausable() || _pauseMenu == nullptr) {
#endif
...
#if defined(DEATH_TARGET_PSP)
		// PSP has no shaders/FBOs and no HUD: render the level's scene root straight to the screen viewport
		// (which uses the camera the app installed) and skip every PlayerViewport/lighting/blur/combine/upscale
		// pass. Unlike headless mode this does NOT set _isHeadless, so textures still load.
		_viewSize = Vector2i(width, height);
		_rootNode->setParent(&theApplication().GetRootNode());
		if (_tileMap != nullptr) {
			_tileMap->OnInitializeViewport();
		}
		// Attach the HUD directly to the app root so the screen viewport draws it in screen space (its Canvas/Font
		// commands carry the PSP UI-space flag and a high layer). There is no PlayerViewport overlay node on PSP.
		if (_hud != nullptr) {
			_hud->setParent(&theApplication().GetRootNode());
		}
		return;
#else
...
#endif
		// PSP has no interactive console overlay, but controller testing chords invoke the same command
		// dispatcher. Keep only the console UI desktop-specific, not the cheat implementations themselves.
		...
		switch (eventType) {
			case EventType::AreaActivateBoss: {
				if (_activeBoss == nullptr && _nextLevelType == ExitType::None) {
					...
					// Some original levels place the boss farther from its trigger than the PSP actor activation
					// window. Activate only the nearest boss on demand instead of widening the window for every actor.
					if (_activeBoss == nullptr && initiator != nullptr) {
						const Vector2f pos = initiator->GetPos();
						if (_eventMap->ActivateNearestBoss((std::int32_t)pos.X / TileSet::DefaultTileSize,
							(std::int32_t)pos.Y / TileSet::DefaultTileSize)) {
							findActiveBoss();
						}
					}
	...
	{
		return SerializeStateToStream(dest, true);
	}

	bool LevelHandler::SerializeCurrentStateToStream(Stream& dest)
	{
		return SerializeStateToStream(dest, false);
	}

	bool LevelHandler::SerializeStateToStream(Stream& dest, bool fromCheckpoint)
	{
		std::uint8_t flags = 0;
		...
		dest.WriteValueAsLE<float>(fromCheckpoint ? _checkpointFrames : GetElapsedFrames());
		...
		_tileMap->SerializeResumableToStream(dest, fromCheckpoint);
		_eventMap->SerializeResumableToStream(dest, fromCheckpoint);

		std::size_t playerCount = 0;
		for (const Actors::Player* player : _players) {
			if (ShouldSerializePlayer(player)) playerCount++;
		}
		dest.WriteValue<std::uint8_t>((std::uint8_t)playerCount);
		for (Actors::Player* player : _players) {
			if (ShouldSerializePlayer(player)) player->SerializeResumableToStream(dest, fromCheckpoint);
		}

		return true;
	}

	bool LevelHandler::ShouldSerializePlayer(const Actors::Player*) const
	{
		return true;
	}
	...
	void LevelHandler::ProcessEvents(float timeMult)
	{
		...
		if (!_players.empty()) {
			...
			constexpr std::int32_t tileSize = TileSet::DefaultTileSize;
			const std::int32_t activationRangeX = (_viewSize.X + tileSize * 2 - 1) / (tileSize * 2) + ActivateTileMargin;
			const std::int32_t activationRangeY = (_viewSize.Y + tileSize * 2 - 1) / (tileSize * 2) + ActivateTileMargin;
			...
			for (std::size_t i = 0; i < playerCount; i++) {
				...
				const auto& activationRange = playerZones.emplace_back(tx - activationRangeX, ty - activationRangeY, tx + activationRangeX, ty + activationRangeY);
				playerZones.emplace_back(activationRange.L - DeactivateTileHysteresis, activationRange.T - DeactivateTileHysteresis,
					activationRange.R + DeactivateTileHysteresis, activationRange.B + DeactivateTileHysteresis);
			}
	...
	void LevelHandler::ResolveCollisions(float timeMult)
	{
		...
		while (it != _actors.end()) {
			...
			if (actor->GetState(Actors::ActorState::IsDirty)) {
				// An actor with no collision proxy has nothing to move, so leave it dirty (a proxy may appear
				// later) and move on. This used to `continue` without advancing the iterator or clearing the
				// flag, which re-examined the same actor forever - a hang, not a skip.
				if (actor->_collisionProxyID != Collisions::NullNode) {
					actor->UpdateAABB();
	...
	{
#if defined(DEATH_TARGET_PSP)
		// No PlayerViewport on PSP (it builds lighting/blur/combine shader passes). The camera is driven
		// directly on the screen viewport by the app, so leave _assignedViewports empty.
		return;
#endif
...
#if defined(DEATH_TARGET_PSP)
				true); // No HUD/weapon-wheel on PSP
#else
...
#else
		// Online sessions keep simulating while their local PSP menu is open.
		_pspSimulationPaused = IsPausable();
#endif
		if (IsPausable()) {
	...
	{
		for (auto* player : _players) {
			player->SetModifier(player->GetModifier() == Actors::Player::Modifier::Airboard
				? Actors::Player::Modifier::None : Actors::Player::Modifier::Airboard);
		}
	}
