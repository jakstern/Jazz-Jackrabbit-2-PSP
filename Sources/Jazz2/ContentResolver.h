...
namespace Jazz2
{
	...
	*/
	...
	public:
		...
		/**
		 * @brief Palette row holding the base sprite palette hue-rotated 180 degrees
		 *
		 * Lets an indexed sprite render as its opposite hue with no second texture, by drawing it through this
		 * @ref paletteOffset (`Hue180PaletteRow * ColorsPerPalette`). Rebuilt from the live base palette in
		 * @ref RecreateGemPalettes so it tracks per-level palette changes. The Electro bullet reuses the blue
		 * Devan energy-bolt sprite through this row to render as an orange bolt.
		 */
		static constexpr std::int32_t Hue180PaletteRow = 3;
		/**
		 * @brief First shared-palette row available for dynamic per-player allocation
		 *
		 * Lower rows are reserved for the sprite palette (row 0), the generated gem palettes (rows 1-2) and the
		 * hue-rotated sprite palette (@ref Hue180PaletteRow).
		 */
		...
		static constexpr std::uint8_t LevelFile = ContentFileType::Level;
		static constexpr std::uint8_t EpisodeFile = ContentFileType::Episode;
		static constexpr std::uint8_t CacheIndexFile = ContentFileType::CacheIndex;
		static constexpr std::uint8_t ConfigFile = ContentFileType::Config;
		static constexpr std::uint8_t StateFile = ContentFileType::State;
		static constexpr std::uint8_t SfxListFile = ContentFileType::SfxList;
		static constexpr std::uint8_t HighscoresFile = ContentFileType::Highscores;
#endif
		...
		/** @brief Unmounts all `.pak` files without scanning for replacements */
		void UnmountPaks();
		...
		// Diagnostics only: resident collision-mask bytes (bit-packed) and cache populations, for the once-per-second
		// PSP memory report. Iterates the graphics cache, so the sampler calls it at most once a second.
		void GetPspMemoryFootprint(std::uint32_t& maskBytes, std::uint16_t& metadataCount,
		 ...
		 * @param forceStreamed  Keep linked PSP pack textures in the bounded streamed residency cache
		 */
		Metadata* RequestMetadata(StringView path, bool forceIndexed = false, bool forceStreamed = false);
		/**
		 ...
		 * @param forceStreamed  Keep this PSP pack texture in the bounded streamed residency cache
		 */
		GenericGraphicResource* RequestGraphics(StringView path, std::uint16_t paletteOffset, bool keepIndexed = false,
			bool forceStreamed = false);
	...
	private:
		...
		static constexpr std::uint32_t IndexedGraphicsCacheKey = UINT16_MAX;
		static constexpr std::uint32_t StreamedGraphicsCacheKey = 1u << 16;
		static constexpr std::uint32_t GetGraphicsCacheKey(std::uint16_t paletteOffset, bool keepIndexed, bool forceStreamed) {
			return (keepIndexed ? IndexedGraphicsCacheKey : paletteOffset) |
				(forceStreamed ? StreamedGraphicsCacheKey : 0u);
		}
		...
		bool _isHeadless;
		bool _isLoading;
		// 16-byte aligned so every 1 KB palette row can be handed straight to sceGuClutLoad
		// on PSP without staging it through a scratch buffer (see PspEmitter EnsureClutLoaded).
		alignas(16) std::uint32_t _palettes[PaletteCount * ColorsPerPalette];
		...
		HashMap<Pair<String, std::uint32_t>, std::unique_ptr<GenericGraphicResource>> _cachedGraphics;
