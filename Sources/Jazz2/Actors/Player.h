	...
	*/
	...
	public:
		...
		*/
		...
		void SerializeResumableToStream(Stream& dest, bool fromCheckpoint = true);
		...
		/** @brief Returns whether the original end-of-level run-off animation is currently playing */
		bool IsPspEndOfLevelTransition() const;
		/** @brief Temporarily draws the run-off actor above the PSP blackout overlay */
		void SetPspEndOfLevelForeground(bool enabled);
#endif
	...
	protected:
...
#if defined(DEATH_TARGET_PSP)
		std::uint16_t _pspEndOfLevelOriginalLayer = 0;
		bool _pspEndOfLevelForeground = false;
#endif
		...
		// The corpse left behind by this player's last death. A player keeps at most one corpse, so spawning a new
		// one retires the previous. Weak, because the level handler owns actors and drops them once destroyed.
		std::weak_ptr<ActorBase> _lastCorpse;
		...
		// Shield decoration animations, loaded on demand the first time a shield is active (a rare event) instead of
		// being pinned in the player metadata. Unshielded play never keeps the large RGBA shield atlases around.
		Metadata* _shieldMetadata = nullptr;
		...
		/** @brief Called when the player dies to leave a corpse behind */
		virtual void SpawnCorpse();
