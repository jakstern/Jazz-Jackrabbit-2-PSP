...
namespace Jazz2::Actors::Multiplayer
{
	...
	{
		// OneUpCollectible only collects itself when this returns true, so refusing here left the pickup sitting
		// in the level permanently - the player walks through it and nothing happens. Extra lives are a no-op in
		// the competitive modes (respawns are unlimited there) but they do count in Cooperation, and either way
		// the pickup should be consumed and scored. The replication for it already exists: RemotePlayerOnServer
		// forwards the new total via HandlePlayerSetLives, and the client applies PlayerPropertyType::Lives.
		return MpPlayer::AddLives(count);
	}
