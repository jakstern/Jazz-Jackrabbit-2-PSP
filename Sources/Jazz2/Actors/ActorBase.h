...
namespace Jazz2::Actors
{
	...
	*/
	...
	public:
		...
		/** @brief Effective fur/recolor colour to replicate for this actor (0 = none). Overridden by actors that
			carry a recolour a plain remote client cannot otherwise learn, e.g. a player corpse. Virtual so the
			server can read it without a `runtime_cast` (the PSP build is `-fno-rtti`). */
		virtual std::uint32_t GetReplicatedFurColor() const { return 0; }
