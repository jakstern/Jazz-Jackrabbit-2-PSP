		...
		if (!_pieces.empty()) {
			...
			if (chainAnim != nullptr && chainAnim->Base->TextureDiffuse != nullptr) {
				...
				for (std::int32_t i = 0; i < _pieces.size(); i++) {
					...
					const Colorf pieceColor = (_shade && scale < 1.0f ? Colorf(scale, scale, scale, 1.0f) : Colorf::White);
					...
					command->SetPspSpriteData(pieceColor.Data(), texScaleX, texBiasX, texScaleY, texBiasY,
						(float)chainAnim->Base->FrameDimensions.X, (float)chainAnim->Base->FrameDimensions.Y,
						chainIndexed ? (float)chainAnim->PaletteOffset : 0.0f);
