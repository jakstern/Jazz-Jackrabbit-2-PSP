	...
	bool Bridge::OnDraw(RenderQueue& renderQueue)
	{
		if (_currentAnimation != nullptr) {
			...
			for (std::int32_t i = 0; i < _pieces.size(); i++) {
				...
				command->SetPspSpriteData(Colorf::White.Data(), texScaleX, texBiasX, texScaleY, texBiasY,
					(float)_currentAnimation->Base->FrameDimensions.X, (float)_currentAnimation->Base->FrameDimensions.Y,
					indexed ? (float)_currentAnimation->PaletteOffset : 0.0f);
