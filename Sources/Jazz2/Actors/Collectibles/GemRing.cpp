		...
		if (!_pieces.empty()) {
			...
			if (res != nullptr && res->Base->TextureDiffuse != nullptr) {
				...
				for (std::int32_t i = 0; i < _pieces.size(); i++) {
					...
					const float spriteWidth = res->Base->FrameDimensions.X * _pieces[i].Scale;
					const float spriteHeight = res->Base->FrameDimensions.Y * _pieces[i].Scale;
					const Colorf pieceColor(1.0f, 1.0f, 1.0f, 0.7f);
					auto instanceBlock = command->GetMaterial().UniformBlock(Material::InstanceBlockName);
					instanceBlock->GetUniform(Material::TexRectUniformName)->SetFloatValue(texScaleX, texBiasX, texScaleY, texBiasY);
					instanceBlock->GetUniform(Material::SpriteSizeUniformName)->SetFloatValue(spriteWidth, spriteHeight);
					instanceBlock->GetUniform(Material::ColorUniformName)->SetFloatVector(pieceColor.Data());
					command->SetPspSpriteData(pieceColor.Data(), texScaleX, texBiasX, texScaleY, texBiasY,
						spriteWidth, spriteHeight, indexed ? (float)res->PaletteOffset : 0.0f);
