...
namespace Jazz2::Actors::Collectibles
{
	...
	void CollectibleBase::OnUpdate(float timeMult)
	{
		...
		if (_untouched) {
			...
			// The idle bob is purely cosmetic (a few pixels around the resting position), but without Force each
			// pickup ran a full tile-collision query every frame just to apply it. That cost scales with the number
			// of pickups on screen, which is what makes dense pickup clusters expensive.
			float waveOffset = 3.2f * cosf((_phase * 0.25f) * fPi) + 0.6f;
			MoveInstantly(Vector2f(_pos.X, _startingY + waveOffset), MoveType::Absolute | MoveType::Force);
