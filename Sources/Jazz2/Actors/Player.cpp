...
namespace Jazz2::Actors
{
		...
		if (_invulnerableTime > 0.0f) {
			...
			if (_invulnerableTime <= 0.0f) {
				...
				if (_shieldSpawnTime <= 0.0f) {
					...
					if (tilemap != nullptr) {
						if (_shieldMetadata == nullptr) {
							_shieldMetadata = ContentResolver::Get().RequestMetadata("Common/PlayerShield"_s);
						}
						auto* res = (_shieldMetadata != nullptr ? _shieldMetadata->FindAnimation(Shield) : nullptr);
	...
	void Player::HandleHorizontalMovement(float timeMult)
	{
		if (_keepRunningTime <= 0.0f) {
			...
			// PSP input already applies a radial deadzone, so retain the full remaining range for slow walking.
			constexpr float MovementInputThreshold = 0.0f;
#else
			constexpr float MovementInputThreshold = 0.4f;
#endif
			...
			} else if (canWalk && playerMovementVelocity > MovementInputThreshold) {
	...
	bool Player::OnDraw(RenderQueue& renderQueue)
	{
		if (_weaponFlareTime > 0.0f && !_inWater && _currentTransition == nullptr) {
			...
			if (res != nullptr && res->Base->TextureDiffuse != nullptr) {
				...
				const Colorf flareColor(1.0f, 1.0f, 1.0f, 1.8f);
				...
				instanceBlock->GetUniform(Material::ColorUniformName)->SetFloatVector(flareColor.Data());
				command->SetPspSpriteData(flareColor.Data(), texScaleX, texBiasX, texScaleY, texBiasY,
					(float)res->Base->FrameDimensions.X, res->Base->FrameDimensions.Y * scaleY);
				command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
		...
		if (_activeShield != ShieldType::None) {
			// Load the shield decoration metadata on first use (shields are rare; keeping them out of the player
			// metadata avoids pinning ~6 MiB of RGBA shield atlases for players who never pick one up).
			if (_shieldMetadata == nullptr) {
				_shieldMetadata = ContentResolver::Get().RequestMetadata("Common/PlayerShield"_s);
			}
			DrawShield(renderQueue, _activeShield, _activeShieldTime, _shieldMetadata,
		...
		float elapsedFrames, Vector2f pos, std::uint16_t baseLayer, std::unique_ptr<RenderCommand> (&shieldRenderCommands)[2])
	{
		...
		// The shield decoration metadata is loaded on demand and can be null if it failed to load (e.g. under memory
		// pressure); the cases below dereference it directly, so bail out rather than crash.
		if (_metadata == nullptr) return;

		#if defined(DEATH_TARGET_PSP)
			auto drawShieldSprite = [&](auto* res, std::int32_t commandIndex, float sizeX, float sizeY,
				const Colorf& color, float animationSpeed) {
				if (res == nullptr || res->Base == nullptr || res->Base->TextureDiffuse == nullptr || res->FrameCount <= 0) return;

				auto& command = _shieldRenderCommands[commandIndex];
				if (command == nullptr) {
					command = std::make_unique<RenderCommand>(RenderCommand::Type::Sprite);
					command->GetMaterial().SetBlendingEnabled(true);
				}
				...
				const Vector2i textureSize = res->Base->TextureDiffuse->GetSize();
				const std::int32_t frame = res->FrameOffset +
					((std::int32_t)(elapsedFrames * animationSpeed) % res->FrameCount);
				const std::int32_t column = frame % res->Base->FrameConfiguration.X;
				const std::int32_t row = frame / res->Base->FrameConfiguration.X;
				const float scaleX = float(res->Base->FrameDimensions.X) / textureSize.X;
				const float biasX = float(res->Base->FrameDimensions.X * column) / textureSize.X;
				const float scaleY = float(res->Base->FrameDimensions.Y) / textureSize.Y;
				const float biasY = float(res->Base->FrameDimensions.Y * row) / textureSize.Y;

				float x = _pos.X - sizeX * 0.5f;
				float y = _pos.Y - sizeY * 0.5f;
				if (!PreferencesCache::UnalignedViewport) {
					x = std::floor(x);
					y = std::floor(y);
				}
				command->SetTransformation(Matrix4x4f::Translation(x, y, 0.0f));
				command->SetLayer(baseLayer + 4);
				command->GetMaterial().SetTexture(*res->Base->TextureDiffuse.get());
				command->SetPspSpriteData(color.Data(), scaleX, biasX, scaleY, biasY, sizeX, sizeY, 0.0f);
				renderQueue.AddCommand(command.get());
			};

			const float shieldAlpha = std::min(_activeShieldTime * 0.01f, 1.0f);
			const float shieldScale = std::min(_activeShieldTime * 0.016f + 0.6f, 1.0f);
			switch (shieldType) {
				case ShieldType::Water: {
					auto* res = _metadata->FindAnimation(ShieldWater);
					if (res != nullptr && res->Base != nullptr) {
						drawShieldSprite(res, 0, res->Base->FrameDimensions.X * shieldScale,
							res->Base->FrameDimensions.Y * shieldScale,
							Colorf(1.0f, 1.0f, 1.0f, 0.62f * shieldAlpha), 0.24f);
					}
					break;
				}
				case ShieldType::Fire: {
					const float size = 64.0f * shieldScale;
					drawShieldSprite(_metadata->FindAnimation(ShieldFire), 0, size, size,
						Colorf(1.0f, 1.0f, 1.0f, 0.82f * shieldAlpha), 0.24f);
					break;
				}
				case ShieldType::Lightning: {
					const float size = 64.0f * shieldScale;
					drawShieldSprite(_metadata->FindAnimation(ShieldLightning), 0, size, size,
						Colorf(1.0f, 1.0f, 1.0f, 0.82f * shieldAlpha), 0.24f);
					break;
				}
				default: break;
		...
		switch (shieldType) {
			case ShieldType::Fire: {
				...
				if (res != nullptr && res->Base->TextureDiffuse != nullptr) {
...

					{
						...
						const Colorf shieldColor(2.0f, 2.0f, 0.8f, 0.9f * shieldAlpha);
						auto instanceBlock = command->GetMaterial().UniformBlock(Material::InstanceBlockName);
						...
						instanceBlock->GetUniform(Material::ColorUniformName)->SetFloatVector(shieldColor.Data());
						command->SetPspSpriteData(shieldColor.Data(),
							frames * -0.008f + _pos.X * PosMultiplier, frames * 0.006f - sinf(frames * 0.006f),
							-sinf(frames * 0.015f), frames * 0.006f + _pos.Y * PosMultiplier, shieldSize, shieldSize);
						command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
					...
					}
					{
						...
						const Colorf shieldColor(2.0f, 2.0f, 1.0f, shieldAlpha);
						auto instanceBlock = command->GetMaterial().UniformBlock(Material::InstanceBlockName);
						instanceBlock->GetUniform(Material::TexRectUniformName)->SetFloatValue(
						...
						command->SetPspSpriteData(shieldColor.Data(),
							frames * 0.006f, sinf(frames * 0.006f) + _pos.Y * PosMultiplier,
							sinf(frames * 0.015f) + _pos.X * PosMultiplier, frames * -0.006f, shieldSize, shieldSize);
						command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
			...
			case ShieldType::Water: {
				...
				if (res != nullptr && res->Base->TextureDiffuse != nullptr) {
					...
					const Colorf shieldColor(1.0f, 1.0f, 1.0f, shieldAlpha);
					auto instanceBlock = command->GetMaterial().UniformBlock(Material::InstanceBlockName);
					instanceBlock->GetUniform(Material::TexRectUniformName)->SetFloatValue(texScaleX, texBiasX, texScaleY, texBiasY);
					...
					instanceBlock->GetUniform(Material::ColorUniformName)->SetFloatVector(shieldColor.Data());
					command->SetPspSpriteData(shieldColor.Data(), texScaleX, texBiasX, texScaleY, texBiasY,
						res->Base->FrameDimensions.X * shieldScale, res->Base->FrameDimensions.Y * shieldScale,
						shieldIndexed ? (float)res->PaletteOffset : 0.0f);
					command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
			...
			case ShieldType::Lightning: {
				...
				if (res != nullptr && res->Base->TextureDiffuse != nullptr) {
...

					{
						...
						command->SetPspSpriteData(shieldColor.Data(),
							frames * -0.008f + _pos.X * PosMultiplier, frames * 0.006f - sinf(frames * 0.006f) + _pos.Y * PosMultiplier,
							-sinf(frames * 0.015f), frames * 0.006f, shieldSize, shieldSize);
						command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
	...
	void Player::SpawnCorpse()
	{
		// A player leaves at most one corpse behind: the previous one is retired as the new one appears. Corpses
		// are never cleaned up otherwise, so without this they accumulate for the whole level - one sprite (and,
		// when recolored, one palette reference) per death.
		if (std::shared_ptr<ActorBase> previous = _lastCorpse.lock()) {
			previous->SetState(ActorState::IsDestroyed, true);
		}

		// Pass the effective fur color (bytes 2..5) so the corpse is recolored like the player
		std::uint32_t furColor = GetEffectiveFurColor();
		...
		_lastCorpse = corpse;
		_levelHandler->AddActor(std::move(corpse));
	}
...
#if defined(DEATH_TARGET_PSP)
	bool Player::IsPspEndOfLevelTransition() const
	{
		return _levelExiting == LevelExitingState::Transition;
	}

	void Player::SetPspEndOfLevelForeground(bool enabled)
	{
		if (enabled == _pspEndOfLevelForeground) return;
		_pspEndOfLevelForeground = enabled;
		if (enabled) {
			_pspEndOfLevelOriginalLayer = _renderer.layer();
			_renderer.setLayer((std::uint16_t)(RenderCommand::PspUiLayerBase + 520));
		} else {
			_renderer.setLayer(_pspEndOfLevelOriginalLayer);
		}
	}
#endif
	...
	void Player::InitializeFromStream(ILevelHandler* levelHandler, Stream& src, std::uint16_t version)
	{
		...
		// PSP slot saves use v5 and preserve the live player rather than rebuilding a
		// checkpoint player. Older engine resumable files retain their original layout.
		if (version >= 5) {
			SetHealth(src.ReadVariableInt32());
			_speed.X = src.ReadValueAsLE<float>();
			_speed.Y = src.ReadValueAsLE<float>();
			_activeShield = (ShieldType)src.ReadValue<std::uint8_t>();
			_activeShieldTime = src.ReadValueAsLE<float>();
			_sugarRushLeft = src.ReadValueAsLE<float>();
		}
	...
	void Player::SerializeResumableToStream(Stream& dest, bool fromCheckpoint)
	{
		const auto& inventory = (fromCheckpoint ? _inventoryCheckpoint : _inventory);
		const Vector2f savedPos = (fromCheckpoint ? _checkpointPos : _pos);
		const float savedLight = (fromCheckpoint ? _checkpointLight : _levelHandler->GetAmbientLight(this));
		...
		dest.WriteValueAsLE<float>(savedPos.X);
		dest.WriteValueAsLE<float>(savedPos.Y);
		dest.WriteValueAsLE<float>(savedLight);
		...
		dest.WriteVariableInt32(inventory.Gems[0]);
		dest.WriteVariableInt32(inventory.Gems[1]);
		dest.WriteVariableInt32(inventory.Gems[2]);
		dest.WriteVariableInt32(inventory.Gems[3]);
		...
		dest.Write(inventory.WeaponAmmo, sizeof(inventory.WeaponAmmo));
		dest.Write(inventory.WeaponUpgrades, sizeof(inventory.WeaponUpgrades));
		if (!fromCheckpoint) {
			dest.WriteVariableInt32(_health);
			dest.WriteValueAsLE<float>(_speed.X);
			dest.WriteValueAsLE<float>(_speed.Y);
			dest.WriteValue<std::uint8_t>((std::uint8_t)_activeShield);
			dest.WriteValueAsLE<float>(_activeShieldTime);
			dest.WriteValueAsLE<float>(_sugarRushLeft);
		}
	}
