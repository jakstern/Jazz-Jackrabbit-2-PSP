...
namespace Jazz2::Actors
{
	namespace
	{
		// One-shot: a bad index means bad animation metadata, and it will repeat every frame once it starts.
		bool g_badMaskIndexReported = false;
		void ReportBadMaskIndex(std::int32_t index)
		{
			if (g_badMaskIndexReported) return;
			g_badMaskIndexReported = true;
#if defined(DEATH_TARGET_PSP) && defined(JAZZ2_PSP_DEBUG)
			char line[128];
			std::snprintf(line, sizeof(line), "mask: NEGATIVE index=%d rejected (would have read mask+%u)",
				(int)index, (unsigned)(((std::uint32_t)index) >> 3));
			nCine::PspDiagnosticsLog(line);
#endif
		}

		// Collision masks are 1 bit per texel (baked as coverage > PspGpu::MaskAlphaThreshold); a set bit is a solid
		// texel. Replaces the old `mask[index] > AlphaThreshold` byte compare after masks were bit-packed (8x smaller).
		// A negative index used to be survivable: the pre-bit-packing code did `p[index] > AlphaThreshold` with a
		// SIGNED index, so it quietly read a few bytes before the buffer and returned a wrong answer. After
		// bit-packing the (std::uint32_t) cast turns index -1 into mask[0x1FFFFFFF] - half a gigabyte past the
		// allocation, i.e. an unconditional data abort on a PSP. Reject it instead, and report the first one so
		// the metadata that produced it can be found rather than silently papered over.
		inline bool MaskBit(const std::uint8_t* mask, std::int32_t index)
		{
			if DEATH_UNLIKELY(index < 0) {
				ReportBadMaskIndex(index);
				return false;
			}
			return ((mask[(std::uint32_t)index >> 3] >> ((std::uint32_t)index & 7)) & 1) != 0;
		}
	}
	...
	void ActorBase::CreateParticleDebrisOnPerish(ParticleDebrisEffect effect, Vector2f speed)
	{
		...
		// Guard against divide-by-zero in the debris loops below (a MIPS integer div by 0 faults).
		if (res->Base->FrameConfiguration.X <= 0 || res->Base->FrameConfiguration.Y <= 0 ||
			res->Base->FrameDimensions.X <= 0 || res->Base->FrameDimensions.Y <= 0) {
			return;
		}
	...
	bool ActorBase::IsCollidingWith(ActorBase* other)
	{
		...
		if (!perPixel1 || !perPixel2) {
			...
			if (perPixel1) {
				...
				std::int32_t frame1 = std::max(0, std::min(_renderer.CurrentFrame, res->FrameCount - 1));
				...
				std::int32_t frame2 = std::max(0, std::min(other->_renderer.CurrentFrame, res->FrameCount - 1));
			...
			for (std::int32_t i = x1; i < x2; i += PerPixelCollisionStep) {
				for (std::int32_t j = y1; j < y2; j += PerPixelCollisionStep) {
					...
					if (MaskBit(p, ((j + dy) * stride) + i1 + dx)) {
						return true;
					}
				}
			}
			...
			std::int32_t frame1 = std::max(0, std::min(_renderer.CurrentFrame, res1->FrameCount - 1));
			...
			std::int32_t frame2 = std::max(0, std::min(other->_renderer.CurrentFrame, res2->FrameCount - 1));
			...
			for (std::int32_t i = x1; i < x2; i += PerPixelCollisionStep) {
				for (std::int32_t j = y1; j < y2; j += PerPixelCollisionStep) {
					...
					if (MaskBit(p1, ((j + dy1) * stride1) + i1 + dx1) && MaskBit(p2, ((j + dy2) * stride2) + i2 + dx2)) {
						return true;
					}
				}
			}
		...
		for (std::int32_t y1 = 0; y1 < height1; y1 += PerPixelCollisionStep) {
			...
			for (std::int32_t x1 = 0; x1 < width1; x1 += PerPixelCollisionStep) {
				...
				if (x2 >= 0 && x2 < width2 && y2 >= 0 && y2 < height2) {
					if (MaskBit(p1, ((y1 + dy1) * stride1) + x1 + dx1) && MaskBit(p2, ((y2 + dy2) * stride2) + x2 + dx2)) {
						return true;
					}
				}
				posIn2 += stepX;
	...
	bool ActorBase::IsCollidingWithAngled(const AABBf& aabb)
	{
		...
		std::int32_t frame = std::max(0, std::min(_renderer.CurrentFrame, res->FrameCount - 1));
		...
		for (std::int32_t y1 = 0; y1 < height; y1 += PerPixelCollisionStep) {
			...
			for (std::int32_t x1 = 0; x1 < width; x1 += PerPixelCollisionStep) {
				...
				if (MaskBit(p, ((y1 + dy) * stride) + x1 + dx) &&
	...
	void ActorBase::ActorRenderer::Initialize(ActorRendererType type)
	{
		...
		// No shaders on PSP: the GU emitter reads the render command's stashed color/texRect/size and samples the
		// unit-0 texture directly (indexed textures via a GU CLUT, keyed on bytes-per-texel). So the palette-aware
		// shaders and the unit-1 palette texture aren't needed. Use the plain Sprite program and skip all
		// palette/shader binding.
		{
			bool shaderChanged = renderCommand_.GetMaterial().SetShaderProgramType(Material::ShaderProgramType::Sprite);
			if (shaderChanged) {
				shaderHasChanged();
				renderCommand_.GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
			}
			// PSP uses the same plain sprite material for every actor render effect. Reset any color left by
			// FrozenMask/Outline when returning to the default state even when the material itself did not change.
			setColor(Colorf::White);
			// The GU emitter bakes indexed (R8/RG8) sprites through `_palettes + paletteOffset`, so record which
			// palette row this sprite samples (e.g. the gem-gradient rows via PaletteOffset). A recolor override
			// (>= 0) wins over the animation's base offset.
			bool hasPalette = (_paletteOffset >= 0 || _baseIndexed);
			setPaletteOffset(hasPalette ? (float)(_paletteOffset >= 0 ? _paletteOffset : _basePaletteOffset) : 0.0f);
			return;
		}
	...
	bool ActorBase::ActorRenderer::IsAnimationRunning()
	{
		...
		switch (LoopMode) {
			case AnimationLoopMode::FixedSingle:
				return false;
			case AnimationLoopMode::Loop:
				// A zero/negative-duration loop cannot advance (AnimTime / AnimDuration would divide by zero, which
				// faults on real PSP hardware); treat it as a static frame instead of entering the update path.
				return !AnimPaused && AnimDuration > 0.0f;
			case AnimationLoopMode::Once:
	...
	void ActorBase::ActorRenderer::UpdateVisibleFrames()
	{
		...
		if (FrameConfiguration.X <= 0) { return; } // avoid divide-by-zero on a missing/empty animation
