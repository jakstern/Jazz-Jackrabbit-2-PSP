...
#if defined(DEATH_TARGET_PSP)
// Max simultaneously-alive destructible debris particles on PSP, and a hard cap on how long any one lives (the
// death effects request up to ~320 frames, which keeps a capped-full list drawing every frame for ~5s). See
// CreateDebris / CreateParticleDebris / CreateSpriteDebris.
// A normal isolated effect still has enough fragments to read clearly, while overlapping kills/explosions stop
// admitting cosmetic debris before they can push a 60 Hz frame over budget.
static constexpr std::size_t PspMaxDebris = 32;
// A single shot can level a whole cluster in one frame (TNT clears a 144x144 px area, up to ~25 tiles). Every
// destroyed tile queues a sound and a debris burst; the sounds are the expensive part, since each one allocates a
// player and feeds a 16-voice software mixer. Only the first few tiles per frame get the full treatment - the rest
// still break, just silently and without debris.
static constexpr std::int32_t PspMaxDestroyedTileEffectsPerFrame = 3;
static constexpr float PspMaxDebrisTime = 120.0f;
static constexpr float PspSimpleDebrisMaxSize = 12.0f;
#endif
...
namespace Jazz2::Tiles
{
	...
	void TileMap::OnUpdate(float timeMult)
	{
		...
		for (auto& animTile : _animatedTiles) {
			...
			const std::uint32_t previousTileIdx = animTile.CurrentTileIdx;
...
#if defined(DEATH_TARGET_PSP)
			if (animTile.CurrentTileIdx != previousTileIdx) _tileVisualRevision++;
#endif
		}
	...
	// Budgets the expensive side effects of destroying a tile (sound + debris) to a few per frame. Returns false once
	// this frame's budget is spent; the tile itself is still destroyed either way.
	bool TileMap::CanSpawnDestroyedTileEffects()
	{
#if defined(DEATH_TARGET_PSP)
		if (_destroyedTileEffectsThisFrame >= PspMaxDestroyedTileEffectsPerFrame) {
			return false;
		}
		_destroyedTileEffectsThisFrame++;
#endif
		return true;
	}
...
#endif
#if defined(DEATH_TARGET_PSP)
		// Called exactly once per frame (unlike OnDraw, which runs per viewport), so this is where the
		// per-frame destruction budget refills.
		_destroyedTileEffectsThisFrame = 0;
#endif
	}
...
#endif
				if (tile.DestructFrameIndex >= max) {
					if (CanSpawnDestroyedTileEffects()) {
						if (!soundName.empty()) {
	...
	void TileMap::DrawLayer(RenderQueue& renderQueue, TileMapLayer& layer, const Rectf& cullingRect, Vector2f viewCenter)
	{
...
#if defined(DEATH_TARGET_PSP)
		// PSP has no shaders/UBO, so only single-tileset layers render (through the batched tile mesh path). That
		// includes the Sky/Circle textured background: the desktop warps its 8x8 tiles with a shader, but we can
		// tile them across the view with parallax instead (see the mesh-mode / repeat handling below). Tinted or
		// per-tile layers and multi-tileset levels can't mesh-render, so skip them.
		const bool pspSkyLayer = (layer.Description.RendererType >= LayerRendererType::Sky && layer.Description.RendererType <= LayerRendererType::Circle);
		if (_tileSets.size() != 1 || (layer.Description.RendererType != LayerRendererType::Default && !pspSkyLayer)) {
			// Fill the view with the layer's horizon colour as a fallback so a clear sky isn't the bare clear colour.
			if (pspSkyLayer) {
				auto* command = RentRenderCommand(LayerRendererType::Solid);
				command->SetType(RenderCommand::Type::TileMap);
				const Vector4f& sky = layer.Description.Color;
				command->SetPspSpriteData(Colorf(sky.X, sky.Y, sky.Z, 1.0f).Data(), 1.0f, 0.0f, 1.0f, 0.0f, cullingRect.W, cullingRect.H);
		...
		if (
#if !defined(DEATH_TARGET_PSP)
			layer.Description.RendererType >= LayerRendererType::Sky && layer.Description.RendererType <= LayerRendererType::Circle && tileCount.Y == 8 && tileCount.X == 8
#else
			false // No shader-warped textured background on PSP; fall through and tile the sky's 8x8 tiles normally.
#endif
		) {
...
#if defined(TILEMAP_USE_SINGLE_DRAW)
#	if defined(DEATH_TARGET_PSP)
			// On PSP any single-tileset layer that reached here (Default or the Sky/Circle background) meshes.
			bool meshMode = (_tileSets.size() == 1);
			// The sky's 8x8 tiles don't set the Repeat flags (the desktop shader tiled them), so force tiling here.
			const bool repeatX = layer.Description.RepeatX || pspSkyLayer;
			const bool repeatY = layer.Description.RepeatY || pspSkyLayer;
#	else
			bool meshMode = (layer.Description.RendererType == LayerRendererType::Default && _tileSets.size() == 1);
			const bool repeatX = layer.Description.RepeatX;
			const bool repeatY = layer.Description.RepeatY;
#	endif
			SmallVector<float, 0>* layerVertices = nullptr;
			#if defined(DEATH_TARGET_PSP)
			PspLayerMeshCache* pspMeshCache = nullptr;
			const float pspMeshOriginX = (!PreferencesCache::UnalignedViewport ? std::floor(x1) : x1);
			const float pspMeshOriginY = (!PreferencesCache::UnalignedViewport ? std::floor(y1) : y1);
			#endif
			if DEATH_LIKELY(meshMode) {
			#if defined(DEATH_TARGET_PSP)
				const std::int32_t layerIndex = (std::int32_t)(&layer - _layers.data());
				while ((std::int32_t)_pspLayerMeshCaches.size() <= layerIndex) _pspLayerMeshCaches.emplace_back();
				pspMeshCache = &_pspLayerMeshCaches[layerIndex];
				const std::int32_t columns = (std::int32_t)std::floor((x3 - x1) / TileSet::DefaultTileSize) + 1;
				const std::int32_t rows = (std::int32_t)std::floor((y3 - y1) / TileSet::DefaultTileSize) + 1;
				const bool cacheHit = (pspMeshCache->VisualRevision == _tileVisualRevision &&
					pspMeshCache->TileAbsX == tileAbsX && pspMeshCache->TileAbsY == tileAbsY &&
					pspMeshCache->Columns == columns && pspMeshCache->Rows == rows &&
					pspMeshCache->RepeatX == repeatX && pspMeshCache->RepeatY == repeatY);
				if (cacheHit) {
					EmitPspCachedLayerMesh(renderQueue, *pspMeshCache, _tileSets[0].Data.get(),
						layer.Description.Color, layer.Description.Depth, pspMeshOriginX, pspMeshOriginY);
					return;
				}

				const Vector2i textureSize = _tileSets[0].Data->TextureDiffuse->GetSize();
				const std::int32_t pagesX = std::max(1, (textureSize.X + nCine::PspGpu::PageTexels - 1) / nCine::PspGpu::PageTexels);
				const std::int32_t pagesY = std::max(1, (textureSize.Y + nCine::PspGpu::PageTexels - 1) / nCine::PspGpu::PageTexels);
				pspMeshCache->PageVertices.resize(pagesX * pagesY);
				for (auto& pageVertices : pspMeshCache->PageVertices) pageVertices.clear();
				pspMeshCache->TileAbsX = tileAbsX;
				pspMeshCache->TileAbsY = tileAbsY;
				pspMeshCache->Columns = columns;
				pspMeshCache->Rows = rows;
				pspMeshCache->VisualRevision = _tileVisualRevision;
				pspMeshCache->RepeatX = repeatX;
				pspMeshCache->RepeatY = repeatY;
			#else
			...
			for (float x2 = x1; x2 <= x3; x2 += TileSet::DefaultTileSize) {
				...
				for (float y2 = y1; y2 <= y3; y2 += TileSet::DefaultTileSize) {
					...
					if DEATH_LIKELY(meshMode) {
					#if defined(DEATH_TARGET_PSP)
						const std::int32_t cellX = tileId % tileSet->TilesPerRow;
						const std::int32_t cellY = tileId / tileSet->TilesPerRow;
						const std::int32_t pagesX = std::max(1, (texSize.X + nCine::PspGpu::PageTexels - 1) / nCine::PspGpu::PageTexels);
						std::int32_t page = (cellY / nCine::PspGpu::PageCells) * pagesX + (cellX / nCine::PspGpu::PageCells);
						page = std::clamp(page, 0, (std::int32_t)pspMeshCache->PageVertices.size() - 1);
						AppendTileQuad(pspMeshCache->PageVertices[page], x2r - pspMeshOriginX, y2r - pspMeshOriginY,
							(float)TileSet::DefaultTileSize, texScaleX, texBiasX, texScaleY, texBiasY, tile.Alpha / 255.0f);
					#else
...
#if defined(TILEMAP_USE_SINGLE_DRAW)
		#if defined(DEATH_TARGET_PSP)
			if DEATH_LIKELY(meshMode && pspMeshCache != nullptr) {
				EmitPspCachedLayerMesh(renderQueue, *pspMeshCache, _tileSets[0].Data.get(),
					layer.Description.Color, layer.Description.Depth, pspMeshOriginX, pspMeshOriginY);
			}
		#else
...
#if defined(DEATH_TARGET_PSP)
		// Guard against a zero chunk size (would make the loop below never advance -> hang).
		if (maxVerticesPerChunk == 0) { maxVerticesPerChunk = 600; }
#endif
...
#if defined(DEATH_TARGET_PSP)
	void TileMap::EmitPspCachedLayerMesh(RenderQueue& renderQueue, PspLayerMeshCache& cache, TileSet* tileSet,
		const Vector4f& layerColor, std::uint16_t depth, float originX, float originY)
	{
		(void)layerColor; // PSP tile tint shaders are not part of the GU path; preserve the established white tint.
		constexpr std::uint32_t FloatsPerVertex = 8;
		constexpr std::uint32_t MaxVerticesPerChunk = 600; // 100 complete quads; comfortably below GU list limits

		for (std::int32_t page = 0; page < (std::int32_t)cache.PageVertices.size(); ++page) {
			SmallVector<float, 0>& vertices = cache.PageVertices[page];
			const std::uint32_t totalVertices = (std::uint32_t)(vertices.size() / FloatsPerVertex);
			for (std::uint32_t firstVertex = 0; firstVertex < totalVertices; firstVertex += MaxVerticesPerChunk) {
				...
				command->SetType(RenderCommand::Type::TileMap);
				command->GetMaterial().SetBlendingFactors(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				command->GetMaterial().SetTexture(0, *tileSet->TextureDiffuse);
				command->SetPspSpriteData(Colorf::White.Data(), 1.0f, 0.0f, 1.0f, 0.0f, 0.0f, 0.0f);
				command->pspTexturePage_ = (std::int16_t)page;

				auto& geometry = command->GetGeometry();
				geometry.SetElementsPerVertex(FloatsPerVertex);
				geometry.SetDrawParameters(GL_TRIANGLES, 0, count);
				geometry.SetHostVertexPointer(vertices.data() + firstVertex * FloatsPerVertex);
				command->SetTransformation(Matrix4x4f::Translation(originX, originY, 0.0f));
				command->SetLayer(depth);
				renderQueue.AddCommand(command);
			}
		}
	...
	{
#if defined(DEATH_TARGET_PSP)
		if (_sprLayerIndex < 0) return;
		// Bound cosmetic work during overlapping effects. Excess fragments are simply not admitted; existing debris
		// remains stable and fades normally through the grouped mesh and simplified-physics paths.
		if (_debrisList.size() >= PspMaxDebris) return;
#endif
		auto& spriteLayer = _layers[_sprLayerIndex];
		if ((debris.Flags & DebrisFlags::Disappear) == DebrisFlags::Disappear && debris.Depth <= spriteLayer.Description.Depth) {
			...
			if (tileSet != nullptr) {
				...
				if (_sprLayerIndex + 1 < (std::int32_t)_layers.size()) {
					auto& nextLayer = _layers[_sprLayerIndex + 1];
					// Index the next layer with ITS OWN stride and bounds - it may be a different size than the
					// sprite layer, and using the sprite layer's stride here reads out of bounds (faults on PSP).
					if (nextLayer.Description.SpeedX == 1.0f && nextLayer.Description.SpeedY == 1.0f &&
						x < nextLayer.LayoutSize.X && y < nextLayer.LayoutSize.Y) {
						tileId = ResolveTileID(nextLayer.Layout[x + y * nextLayer.LayoutSize.X]);
						if (tileSet->IsTileFilled(tileId)) {
							return;
						}
					}
...
#if defined(DEATH_TARGET_PSP)
		// Tiny fragments do not need pixel-mask-accurate collision. A single coarse tile-cell query preserves a
		// recognizable floor bounce while avoiding the multiple AABB/mask queries of the actor-grade collision path.
		if (_debrisList.back().Size.X <= PspSimpleDebrisMaxSize && _debrisList.back().Size.Y <= PspSimpleDebrisMaxSize) {
			_debrisList.back().Flags |= DebrisFlags::SimplifiedPhysics;
		}
		// Clamp lifetime so the death-effect loops (Fire/Dissolve/Water request ~280-340 frames) don't keep a
		// capped-full particle list drawing every frame for seconds. One place covers all CreateDebris callers.
		if (_debrisList.back().Time > PspMaxDebrisTime) {
			_debrisList.back().Time = PspMaxDebrisTime;
		}
#endif
	}
	...
	void TileMap::CreateParticleDebris(const GraphicResource* res, Vector3f pos, Vector2f force, std::int32_t currentFrame, bool isFacingLeft)
	{
...
#if defined(DEATH_TARGET_PSP)
		// The GU draws one sprite per particle and UpdateDebris fully simulates each every frame, so the desktop
		// density (a particle every DebrisSize+1 px) is unaffordable. Emit ~1/9 as many (3x coarser grid), grow
		// each to cover the gap, and give them a much shorter life so the steady-state list stays tiny.
		constexpr std::int32_t DebrisStep = (DebrisSize + 1) * 3;
		constexpr float DebrisTime = 80.0f;
		constexpr float DebrisSizeScale = 3.0f;
#else
		constexpr std::int32_t DebrisStep = DebrisSize + 1;
		constexpr float DebrisTime = 320.0f;
		constexpr float DebrisSizeScale = 1.0f;
#endif
		...
		for (std::int32_t fy = 0; fy < res->Base->FrameDimensions.Y; fy += DebrisStep) {
			for (std::int32_t fx = 0; fx < res->Base->FrameDimensions.X; fx += DebrisStep) {
#if defined(DEATH_TARGET_PSP)
				if (_debrisList.size() >= PspMaxDebris) return;
#endif
				float currentSize = DebrisSize * DebrisSizeScale * Random().FastFloat(0.2f, 1.1f);
				...
				debris.Flags = DebrisFlags::Bounce | DebrisFlags::SimplifiedPhysics;
			}
		}
	}
	...
	void TileMap::CreateSpriteDebris(const GraphicResource* res, Vector3f pos, std::int32_t count)
	{
		...
		for (std::int32_t i = 0; i < count; i++) {
			...
			// Guard divide-by-zero: Random().Next(0, 0) does `% 0`, and `% FrameConfiguration.X` faults if the
			// debris animation has a zero frame count / configuration (a MIPS integer div by 0 faults on PSP).
			std::int32_t curAnimFrame = res->FrameOffset + (res->FrameCount > 0 ? Random().Next(0, res->FrameCount) : 0);
			std::int32_t framesPerRow = (res->Base->FrameConfiguration.X > 0 ? res->Base->FrameConfiguration.X : 1);
			std::int32_t col = curAnimFrame % framesPerRow;
			std::int32_t row = curAnimFrame / framesPerRow;
...
#if defined(DEATH_TARGET_PSP)
				if ((debris.Flags & DebrisFlags::SimplifiedPhysics) == DebrisFlags::SimplifiedPhysics) {
					// Old-style particle approximation: test only the destination tile cell. It deliberately ignores
					// slope masks and exact wall contact; small visual fragments do not justify actor collision cost.
					const std::int32_t tx = (std::int32_t)std::floor(nx / TileSet::DefaultTileSize);
					const std::int32_t ty = (std::int32_t)std::floor(ny / TileSet::DefaultTileSize);
					if (!IsTileEmpty(tx, ty)) {
						if ((debris.Flags & DebrisFlags::Disappear) == DebrisFlags::Disappear) {
							debris.ScaleSpeed = -0.02f;
							debris.AlphaSpeed = -0.08f;
							debris.Speed = Vector2f::Zero;
							debris.Acceleration = Vector2f::Zero;
						} else if (debris.Speed.Y > 0.0f) {
							debris.Speed.Y *= -0.45f;
							debris.Speed.X *= 0.85f;
							debris.AngleSpeed *= 0.85f;
						} else {
							debris.Speed.X *= -0.45f;
							debris.AngleSpeed *= -0.45f;
						}
					}
				} else
#endif
	...
	void TileMap::DrawDebris(RenderQueue& renderQueue)
	{
...
#if defined(DEATH_TARGET_PSP) && defined(TILEMAP_USE_SINGLE_DRAW)
		// Jazz debris is already a flat simulation array; turning every element back into a full engine render
		// command defeats that simplicity. Gather compatible debris into host meshes here so queue sorting, material
		// commit and GU emission happen once per texture/layer/blend group instead of once per particle.
		struct DebrisGroup {
		...
		SmallVector<DebrisGroup, 8> groups;

		auto appendVertex = [](SmallVector<float, 0>& vertices, float x, float y, float u, float v, float alpha) {
			vertices.push_back(x); vertices.push_back(y);
			vertices.push_back(u); vertices.push_back(v);
			vertices.push_back(1.0f); vertices.push_back(1.0f); vertices.push_back(1.0f); vertices.push_back(alpha);
		};

		for (const auto& debris : _debrisList) {
			if (!viewportRect.Contains(debris.Pos) || debris.DiffuseTexture == nullptr) continue;

			const bool additive = ((debris.Flags & DebrisFlags::AdditiveBlending) == DebrisFlags::AdditiveBlending);
			std::int32_t groupIndex = -1;
			for (std::int32_t i = 0; i < (std::int32_t)groups.size(); ++i) {
				const DebrisGroup& group = groups[i];
				if (group.TextureDiffuse == debris.DiffuseTexture && group.PaletteOffset == debris.PaletteOffset &&
					group.Depth == debris.Depth && group.Additive == additive) {
					groupIndex = i;
					break;
				}
			}
			...
			if (groupIndex < 0) {
				...
				const std::int32_t verticesIndex = _debrisMeshVerticesCount++;
				_debrisMeshVertices[verticesIndex].clear();
				groups.push_back(DebrisGroup{ debris.DiffuseTexture, debris.PaletteOffset, debris.Depth, additive, verticesIndex });
				groupIndex = (std::int32_t)groups.size() - 1;
			}

			SmallVector<float, 0>& vertices = _debrisMeshVertices[groups[groupIndex].VerticesIndex];
			const float halfW = debris.Size.X * debris.Scale * 0.5f;
			const float halfH = debris.Size.Y * debris.Scale * 0.5f;
			const float c = (debris.Angle == 0.0f ? 1.0f : std::cos(debris.Angle));
			const float s = (debris.Angle == 0.0f ? 0.0f : std::sin(debris.Angle));
			auto rotateX = [&](float x, float y) { return debris.Pos.X + c * x - s * y; };
			auto rotateY = [&](float x, float y) { return debris.Pos.Y + s * x + c * y; };

			const float x0 = rotateX(-halfW, -halfH), y0 = rotateY(-halfW, -halfH);
			const float x1 = rotateX( halfW, -halfH), y1 = rotateY( halfW, -halfH);
			const float x2 = rotateX( halfW,  halfH), y2 = rotateY( halfW,  halfH);
			const float x3 = rotateX(-halfW,  halfH), y3 = rotateY(-halfW,  halfH);
			const float u0 = debris.TexBiasX, u1 = debris.TexBiasX + debris.TexScaleX;
			const float v0 = debris.TexBiasY, v1 = debris.TexBiasY + debris.TexScaleY;

			appendVertex(vertices, x0, y0, u0, v0, debris.Alpha);
			appendVertex(vertices, x1, y1, u1, v0, debris.Alpha);
			appendVertex(vertices, x2, y2, u1, v1, debris.Alpha);
			appendVertex(vertices, x0, y0, u0, v0, debris.Alpha);
			appendVertex(vertices, x2, y2, u1, v1, debris.Alpha);
			appendVertex(vertices, x3, y3, u0, v1, debris.Alpha);
		}

		for (const DebrisGroup& group : groups) {
			SmallVector<float, 0>& vertices = _debrisMeshVertices[group.VerticesIndex];
			if (vertices.empty()) continue;
			if (_debrisMeshCommandCount >= (std::int32_t)_debrisMeshCommands.size()) {
				auto& newCommand = _debrisMeshCommands.emplace_back(std::make_unique<RenderCommand>());
				newCommand->GetMaterial().SetBlendingEnabled(true);
			}
			RenderCommand* command = _debrisMeshCommands[_debrisMeshCommandCount++].get();
			command->SetType(RenderCommand::Type::Particle);
			command->GetMaterial().SetBlendingFactors(GL_SRC_ALPHA, group.Additive ? GL_ONE : GL_ONE_MINUS_SRC_ALPHA);
			command->GetMaterial().SetTexture(0, *group.TextureDiffuse);
			command->SetPspSpriteData(Colorf::White.Data(), 1.0f, 0.0f, 1.0f, 0.0f, 0.0f, 0.0f,
				(group.PaletteOffset >= 0 ? (float)group.PaletteOffset : 0.0f));

			auto& geometry = command->GetGeometry();
			geometry.SetElementsPerVertex(8);
			geometry.SetDrawParameters(GL_TRIANGLES, 0, (GLsizei)(vertices.size() / 8));
			geometry.SetHostVertexPointer(vertices.data());
			command->SetTransformation(Matrix4x4f::Identity);
			command->SetLayer(group.Depth);
			renderQueue.AddCommand(command);
		}
		return;
#endif
		...
		for (const auto& debris : _debrisList) {
			...
			auto instanceBlock = command->GetMaterial().UniformBlock(Material::InstanceBlockName);
#if defined(DEATH_TARGET_PSP)
			// No live uniform block on PSP - stash the sprite params directly on the command for the GU emitter
			// (same as BaseSprite::updateRenderCommand). Debris renders axis-aligned (no rotation), which is fine.
			command->SetPspSpriteData(Colorf(1.0f, 1.0f, 1.0f, debris.Alpha).Data(),
				debris.TexScaleX, debris.TexBiasX, debris.TexScaleY, debris.TexBiasY, debris.Size.X, debris.Size.Y,
				(debris.PaletteOffset >= 0 ? (float)debris.PaletteOffset : 0.0f));
#else
...
#if defined(DEATH_TARGET_PSP)
			// `instanceBlock` is null on PSP (no GL uniform-block reflection); dereferencing it into BindSpritePalette
			// is a null-`this` fault. The GU emitter reads texture 0 and applies the global palette CLUT itself, so
			// bind the diffuse texture directly - the per-sprite palette offset isn't wired through the emitter yet.
