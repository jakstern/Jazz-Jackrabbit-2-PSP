...
namespace Jazz2::Compatibility
{
	...
	*/
	...
	public:
		...
		/** @brief Converts the data file and writes the result to a `.pak` file. `sink` (optional) receives each
			converted menu image so the PSP asset baker can bake it directly (see JJ2Anims::SpriteSink). */
		void Convert(PakWriter& pakWriter, JJ2Version version, JJ2Anims::SpriteSink sink = nullptr, void* sinkCtx = nullptr);

	private:
		void ConvertSfxList(const Item& item, PakWriter& pakWriter, StringView targetPath, AnimSetMapping& animMapping);
		void ConvertMenuImage(const Item& item, PakWriter& pakWriter, StringView targetPath, std::int32_t width, std::int32_t height, JJ2Anims::SpriteSink sink = nullptr, void* sinkCtx = nullptr);
		void ConvertPicture(const Item& item, PakWriter& pakWriter, StringView targetPath, JJ2Anims::SpriteSink sink = nullptr, void* sinkCtx = nullptr);
		void ConvertRgbaImage(const std::uint8_t* pixels, std::int32_t width, std::int32_t height,
