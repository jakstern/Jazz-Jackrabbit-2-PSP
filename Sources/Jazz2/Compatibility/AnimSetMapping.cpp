...
namespace Jazz2::Compatibility
{
	...
	AnimSetMapping AnimSetMapping::GetAnimMapping(JJ2Version version)
	{
		...
		if (version == JJ2Version::PlusExtension) {
			...
			m.NextSet();
			m.DiscardItems(2); // Normal medium/small menu fonts
			m.Add("UI"_s, "loading_font_large"_s, JJ2DefaultPalette::Menu, true);
			...
			m.NextSet(1, JJ2Version::TSF | JJ2Version::CC);
			m.NextSet();
			m.Add("UI"_s, "multiplayer_char"_s, JJ2DefaultPalette::Menu, true);

			m.Add("UI"_s, "multiplayer_color"_s, JJ2DefaultPalette::Menu, true);
			...
			// Retail base-game menu logo used by the PSP menu. Version-specific title strips and the
			// remaining small/seasonal variants stay unused so every supported data set has the same UI.
			m.DiscardItems(2); // Alternate medium and small menu fonts
			m.Add("UI"_s, "logo_large"_s, JJ2DefaultPalette::Menu, true);
			m.DiscardItems(1, JJ2Version::TSF | JJ2Version::CC); // Version-specific title strip
			m.DiscardItems(1, JJ2Version::CC); // Menu snow
			m.DiscardItems(1); // Small logo
			m.DiscardItems(2, JJ2Version::CC); // CC title variants
