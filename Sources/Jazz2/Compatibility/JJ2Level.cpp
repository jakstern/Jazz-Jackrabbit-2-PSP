...
namespace Jazz2::Compatibility
{
	...
	void JJ2Level::Convert(StringView targetPath, EventConverter& eventConverter, Function<LevelToken(StringView)>&& levelTokenConversion)
	{
		...
		bool hasTreasures = false;
		auto checkTreasure = [&hasTreasures](EventType type) {
			hasTreasures = hasTreasures || type == EventType::Gem || type == EventType::GemGiant ||
				type == EventType::GemRing || type == EventType::CrateGem || type == EventType::BarrelGem;
		};
		for (std::int32_t y = 0; y < _layers[3].Height; y++) {
			for (std::int32_t x = 0; x < _layers[3].Width; x++) {
				...
				checkTreasure(tileEvent.Converted.Type);
			}
		}
		...
		for (std::size_t i = 0; i < _offGridEvents.size(); i++) {
			...
			checkTreasure(offGridEvent.Converted.Type);
		}
