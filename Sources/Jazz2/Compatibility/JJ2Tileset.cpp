...
namespace Jazz2::Compatibility
{
	...
	bool JJ2Tileset::Convert(StringView targetPath, bool includeImage,
		ImageSink imageSink, void* imageSinkContext) const
	{
		...
		DEATH_ASSERT(so->IsValid(), "Cannot open file for writing", false);

		// 0x80 marks a PSP metadata sidecar whose diffuse image lives exclusively in texture.pak.
		const std::uint8_t flags = 0x20 | 0x40 | (includeImage ? 0 : 0x80);
		...
		// Normalize the palette once for both the metadata sidecar and the PSP texture pack sink.
		...
		// The PSP sidecar metadata compresses to only a few KiB (the retail maximum is below 5 KiB).
		// Avoid reserving a full MiB alongside the decoded tile array on memory-constrained hardware.
		MemoryStream ms(64 * 1024);
		{
			DeflateWriter co(ms);
		...
		if (imageSink != nullptr && !imageSink(imageSinkContext, palette, pixels.get(), width, height,
			channelCount, tileCount)) return false;
		if (includeImage) JJ2Anims::WriteImageContent(*so, pixels.get(), width, height, channelCount);
		return so->IsValid();
	}
}
