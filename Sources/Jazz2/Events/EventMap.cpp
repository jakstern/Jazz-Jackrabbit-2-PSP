...
{
	static bool IsSupportedBossEvent(EventType eventType)
	{
		switch (eventType) {
	...
	bool EventMap::ActivateNearestBoss(std::int32_t tx, std::int32_t ty)
	{
		EventTile* nearest = nullptr;
		std::int32_t nearestX = 0;
		std::int32_t nearestY = 0;
		std::int64_t nearestDistance = 0;

		for (std::int32_t y = 0; y < _layoutSize.Y; y++) {
			for (std::int32_t x = 0; x < _layoutSize.X; x++) {
				EventTile& tile = _eventLayout[x + y * _layoutSize.X];
				if (tile.IsEventActive || !IsSupportedBossEvent(tile.Event)) {
					continue;
				}

				const std::int64_t dx = x - tx;
				const std::int64_t dy = y - ty;
		...
		nearest->IsEventActive = true;
		Actors::ActorState flags = Actors::ActorState::IsCreatedFromEventMap | nearest->EventFlags;
		std::shared_ptr<Actors::ActorBase> actor = _levelHandler->EventSpawner()->SpawnEvent(
			nearest->Event, nearest->EventParams, flags, nearestX, nearestY, ILevelHandler::SpritePlaneZ);
		if (actor == nullptr) {
			nearest->IsEventActive = false;
			return false;
		}

		_levelHandler->AddActor(std::move(actor));
		return true;
	}
	...
	{
		// Bounds-checked like GetEventByPosition() below, which has always done it. Callers pass an actor's
		// origin tile, derived from an arbitrary spawn position and never clamped, so it can legitimately land
		// outside the layout - an actor spawned at or past the level edge, or one whose position was set before
		// it was placed. Indexing straight in is a hard fault on PSP, not a harmless garbage read.
		static const EventTile OutOfBounds{};
		if (x < 0 || y < 0 || x >= _layoutSize.X || y >= _layoutSize.Y) {
			return OutOfBounds;
		}
