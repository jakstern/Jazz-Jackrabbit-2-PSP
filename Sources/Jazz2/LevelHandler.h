...
namespace Jazz2
{
	...
	*/
	...
	public:
		...
		/** @brief Number of tiles activated beyond each viewport edge */
		static constexpr std::int32_t ActivateTileMargin = 8;
		/** @brief Additional active-tile margin before actors are deactivated */
		static constexpr std::int32_t DeactivateTileHysteresis = 4;
		...
		virtual void PauseForPspMenu();
		virtual void ResumeFromPspMenu();
		void SetPspMenuBackgroundSimulation(bool enabled);
		virtual void SuppressInputUntilRelease();
		virtual bool IsPspNormalLevelExitActive() const;
		virtual void StartPspMusicAfterFirstPresent();
		virtual void ApplyPspMusicVolume();
		virtual bool HasPspLevelTransition() const;
		virtual void PspCheatSkipForward();
		virtual void PspCheatSkipTo(StringView levelName);
		virtual bool PspInvokeCheat(StringView command);
		virtual ShieldType PspCheatCycleShield();
		virtual void SetPspFlyCheat(bool enabled);
		virtual void SetPspGodCheat(bool enabled);
		...
		/** @brief Serializes the live state instead of the last checkpoint (used by PSP save slots) */
		bool SerializeCurrentStateToStream(Stream& dest);
	...
	protected:
		bool SerializeStateToStream(Stream& dest, bool fromCheckpoint);
		/** @brief Returns whether a player belongs in a serialized world snapshot */
		virtual bool ShouldSerializePlayer(const Actors::Player* player) const;
		...
		// PSP owns its pause overlay outside LevelHandler. Local sessions freeze;
		// online sessions keep the authoritative simulation running with neutral input.
		bool _pspSimulationPaused;
		bool _pspMenuActive = false;
		bool _pspSuppressInputUntilRelease = true;
		bool _pspFlyCheat = false;
		bool _pspGodCheat = false;
#endif
