...
namespace Jazz2::UI::Multiplayer
{
	...
	bool MpInGameCanvasLayer::OnDraw(RenderQueue& renderQueue)
	{
		...
		// PSP fonts are emitted in screen space so HUD text stays fixed while the scene camera moves. Convert the
		// world-space actor positions here; otherwise a nametag follows the camera instead of its actor.
		Vector2f cameraOrigin = Vector2f::Zero;
		if (const Camera* camera = theApplication().GetScreenViewport().GetCamera()) {
			cameraOrigin = camera->GetViewValues().position;
		}
		...
		if (_levelHandler->_isServer) {
			for (auto& [peer, peerDesc] : *_levelHandler->_networkManager->GetPeers()) {
				if (peerDesc->RemotePeer && peerDesc->Player && peerDesc->Player->_renderer.isEnabled()) {
					auto pos = peerDesc->Player->GetPos() - cameraOrigin;
			...
			for (auto& [actorId, playerName] : _levelHandler->_playerNames) {
				...
				if (it != _levelHandler->_remoteActors.end() && it->second->_renderer.isEnabled()) {
					auto pos = it->second->GetPos() - cameraOrigin;
