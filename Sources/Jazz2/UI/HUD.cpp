...
namespace Jazz2::UI
{
	...
	void HUD::OnUpdate(float timeMult)
	{
		...
		// PSP has a single screen (no PlayerViewport), but the coin/gem/boss notification timers still advance.
		{
			if (_coinsTime >= 0.0f) {
				_coinsTime += timeMult;
			}
	...
	bool HUD::OnDraw(RenderQueue& renderQueue)
	{
		...
		// PSP has a single screen (no PlayerViewport / split-screen), so draw the HUD for the controlled
		// player using the full screen as the scoped view.
		if (!players.empty()) {
			Actors::Player* player = players[0];
			Rectf scopedView = view;
			Rectf adjustedScopedView = adjustedView;
	...
	void HUD::OnDrawLevelText(int32_t& charOffset)
	{
		...
		if (_levelTextTime < TransitionTime) {
			const float transition = (TransitionTime - _levelTextTime) / 12.0f;
			offset = transition * transition * transition;
		} else if (_levelTextTime > TransitionTime + StillTime) {
			const float transition = (_levelTextTime - TransitionTime - StillTime) / 12.0f;
			offset = -(transition * transition * transition);
		} else {
			offset = 0;
		}
		...
		// PSP still pays the render-queue cost for every glyph before its emitter can batch them. Keep the
		// animated foreground, but omit the low-opacity duplicate so long sign messages submit half as many commands.
	...
	{
		bool isConsoleVisible = false; // No console (or touch input) on PSP
