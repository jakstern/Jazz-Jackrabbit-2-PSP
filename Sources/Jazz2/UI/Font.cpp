...
namespace Jazz2::UI
{
		...
		: _baseSpacing(0)
	{
		...
		if (texLoader->hasLoaded()) {
			...
			// Pack the desktop Colorized shader's two independent inputs into one T8 index: four bits of
			// source coverage and four bits of its deliberately over-bright grayscale (range 0..1.5).
			// The PSP emitter supplies the selected rainbow dye through a generated 256-entry CLUT.
			auto colorPixels = std::make_unique<std::uint8_t[]>(w * h);
			for (std::int32_t i = 0; i < w * h; i++) {
				const std::uint8_t* src = pixels + i * ContentResolver::PixelSize;
				const float gray = (src[0] + src[1] + src[2]) / 510.0f;
				const std::uint8_t luminance = (std::uint8_t)std::clamp((int)std::lround(gray * 10.0f), 0, 15);
				const std::uint8_t coverage = (std::uint8_t)((src[3] * 15 + 127) / 255);
				colorPixels[i] = (std::uint8_t)((coverage << 4) | luminance);
			}
			_pspColorTexture = std::make_unique<Texture>(path.data(), Texture::Format::R8, w, h);
			_pspColorTexture->LoadFromTexels(colorPixels.get(), 0, 0, w, h);
			_pspColorTexture->SetMinFiltering(SamplerFilter::Nearest);
			_pspColorTexture->SetMagFiltering(SamplerFilter::Nearest);
#endif
		}
	}
	...
	void Font::DrawString(Canvas* canvas, StringView text, std::int32_t& charOffset, float x, float y, std::uint16_t z, Alignment align, Colorf color, float scale, float angleOffset, float varianceX, float varianceY, float speed, float charSpacing, float lineSpacing, bool pspShine)
	{
		std::size_t textLength = text.size();
		if (textLength == 0 || _charSize.Y <= 0) {
			return;
		...
		do {
			...
			if (cursor.first() == '\n') {
				...
				if (uvRect.W > 0 && uvRect.H > 0) {
					...
					// Separate alpha blend so text shadows accumulate correct coverage. On PSP the GU state is handled by
					// the emitter, but retaining the material factors also keeps queue sorting equivalent to other targets.
...
#else
					// PSP consumes the glyph parameters directly in PspEmitter. Avoid constructing desktop shader state and
					// writing fake uniform blocks for every glyph; long sign messages otherwise do this twice (shadow + text)
					// every frame even though none of those values reach the GU.
					command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
#endif
					...
					#if defined(DEATH_TARGET_PSP)
					const bool pspColorized = ((useRandomColor || pspShine) && !isShadow && _pspColorTexture != nullptr);
					command->GetMaterial().SetTexture(*(pspColorized ? _pspColorTexture : _texture).get());
					#else
					command->GetMaterial().SetTexture(*_texture.get());
					#endif

#if defined(DEATH_TARGET_PSP)
					// No live uniform block on PSP - stash the glyph quad's color/texRect/size for the GU emitter
					// (the font texture is plain RGBA, so no palette offset).
					command->SetPspSpriteData(glyphColor.Data(), texCoords.X, texCoords.Y, texCoords.Z, texCoords.W,
						charWidth * glyphScale, uvRect.H * glyphScale);
					command->pspFontColorized_ = pspColorized;
					command->pspFontRainbow_ = (pspColorized && useRandomColor);
					command->pspUiSpace_ = true;
					command->SetLayer((std::uint16_t)(command->GetLayer() + nCine::RenderCommand::PspUiLayerBase));
					command->GetGeometry().SetDrawParameters(GL_TRIANGLE_STRIP, 0, 4);
#endif
