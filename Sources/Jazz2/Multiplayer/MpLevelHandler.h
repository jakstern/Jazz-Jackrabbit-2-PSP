...
namespace Jazz2::Multiplayer
{
	...
	*/
	...
	public:
		...
		bool IsInitialSyncReady() const;
		struct ActorUpdateMetrics {
			std::uint32_t CandidateActors = 0;
			std::uint32_t EmittedActors = 0;
			std::uint32_t PacketBytes = 0;
			std::uint32_t SerializeMicros = 0;
			std::uint32_t Samples = 0;
			bool ForceResync = false;
		};
		const ActorUpdateMetrics& GetActorUpdateMetrics() const { return _actorUpdateMetrics; }
		...
		bool IsPspNormalLevelExitActive() const override;
		...
		/** @brief Requests connected clients to replace their current level before a host-side saved-state load */
		void NotifyPeersOfLevelLoad(StringView levelName, ExitType exitType = ExitType::Normal);
		...
		/** @brief Captures character-independent progression for a connected player (server-side) */
		bool CapturePeerState(const Peer& peer, PlayerCarryOver& carryOver, std::int32_t& health) const;
	...
	protected:
		...
		bool ShouldSerializePlayer(const Actors::Player* player) const override;
	...
	private:
		...
		ExitType _remoteLevelExitType;	// Client: exit presentation announced by PlayerWarpIn
		bool _isServer;
		...
		std::uint32_t _nextSpawnPointIndex;	// Server: rotates spawn points so simultaneous spawns don't stack
		...
		float _waitingForReadyTimeLeft;		// Server: frames left before starting without a peer that never finishes loading
		float _levelReadyRetryTimeout;		// Client: frames until this peer re-reports that it finished loading
		...
		ActorUpdateMetrics _actorUpdateMetrics;
		...
		/** Server: true once every connected peer has actually spawned into the level */
		// Server: how many more players the round is still waiting for, derived from live state (see the
		// definition). Replaces the old AreAllPeersReadyToStart() + latched-count pair with one number.
		std::int32_t MinPlayersToPlay() const;
		std::int32_t GetWaitingForPlayerCount();
		void StartCountdown();
		...
		bool HandleServerPacketClearRemoteActors(const Peer& peer, ArrayView<const std::uint8_t> data);

		// Client: tells the server this peer finished loading the level. Re-sent until the server responds,
		// because the report is silently discarded if the server is not in a level yet.
		void SendLevelReady();
