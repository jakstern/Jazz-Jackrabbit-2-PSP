...
namespace Jazz2::Multiplayer
{
	...
	// Upper bound on holding a round for a peer that never finishes joining. Only ever overrides "someone is
	// still loading" - never MinPlayerCount, so it cannot start a match with nobody to play against.
	static constexpr float MaxWaitingForReadySecs = 30.0f;
	...
	MpLevelHandler::MpLevelHandler(IRootController* root, NetworkManager* networkManager, MpLevelHandler::LevelState levelState, bool enableLedgeClimb)
		: LevelHandler(root), _networkManager(networkManager), _updateTimeLeft(1.0f), _gameTimeLeft(0.0f),
			_levelState(LevelState::InitialUpdatePending), _remoteLevelExitType(ExitType::None), _forceResyncPending(true), _enableSpawning(true), _enqueuedPlaylistChange(false), _lastSpawnedActorId(-1), _waitingForPlayerCount(0), _waitingForReadyTimeLeft(0.0f), _levelReadyRetryTimeout(0.0f), _nextSpawnPointIndex(0),
	...
	bool MpLevelHandler::Initialize(const LevelInitialization& levelInit)
	{
		...
		_console->setParent(GetHudParentNode());
			...
			if (!IsLocalSession()) {
				...
				for (auto& [peer, peerDesc] : *_networkManager->GetPeers()) {
					...
					if (peerDesc->RemotePeer) {
						...
						// The body this confirmed belonged to the previous level and is gone; carrying the flag
						// over would let the next round start before the guest actually has a player again.
						peerDesc->PlayerBodyConfirmed = false;
					}
				}
			}
		}
	...
	void MpLevelHandler::SendLevelReady()
	{
		// Re-armed on every send: this is re-reported until the server proves it heard us (see the caller in
		// OnUpdate), so the interval is also the retry interval.
		_levelReadyRetryTimeout = FrameTimer::FramesPerSecond;
	...
	bool MpLevelHandler::IsInitialSyncReady() const
	{
		// Actor creation and the forced full update use the ordered main channel. Once the
		// first update arrives, the client has consumed the complete initial actor stream.
		return _isServer || (!_players.empty() && _lastUpdated > 0);
	}
	...
	bool MpLevelHandler::IsPspNormalLevelExitActive() const
	{
		if (LevelHandler::IsPspNormalLevelExitActive()) {
			return true;
		}

		return (!_isServer && GetGameMode() == MpGameMode::Cooperation &&
			(_remoteLevelExitType & ExitType::FastTransition) != ExitType::FastTransition &&
			(_remoteLevelExitType & ExitType::TypeMask) == ExitType::Normal);
	}
	...
	void MpLevelHandler::OnBeginFrame()
	{
		...
		if (_isServer) {
			...
			if (input.PressedActions != input.PressedActionsLast) {
				...
				// Reliable, unlike the position updates below, because this is a *change* and not a snapshot: it
				// is only sent when the button mask differs from last frame, so nothing re-states it afterwards.
				// On the unreliable channel a single lost datagram left the server's idea of the held buttons
				// wrong until the player happened to press something else - a dropped "release" meant the
				// character kept running or firing on its own. Rare and tiny (only on change), so ordering it
				// behind the reliable stream costs nothing.
				_networkManager->SendTo(AllPeers, NetworkChannel::Main, (std::uint8_t)ClientPacketType::PlayerKeyPress, packet);
			}
		}
	}
	...
	void MpLevelHandler::OnEndFrame()
	{
		...
		if (_isServer) {
			...
			// Keep reporting until the world actually arrives. LevelReady is the only thing that tells the server
			// this peer finished loading, and it is thrown away unless the server happens to be in a level at that
			// exact moment: the app drops every packet while levelHandler_ is null (menu / mid-load), and the server
			// rebuilds every PeerDescriptor from scratch when it does load, discarding whatever it had recorded. A
			// guest that finishes loading BEFORE the host therefore reported into the void and sat on "Synchronizing"
			// forever, while a slower console reporting after the host was up worked fine.
			// No acknowledgement packet is needed: IsInitialSyncReady() IS the acknowledgement, inferred from
			// traffic the server already has to send us. The server's handler is idempotent (it only ever raises
			// LevelState), so a duplicate report costs nothing.
			if (_levelState != LevelState::InitialUpdatePending && !IsInitialSyncReady()) {
				_levelReadyRetryTimeout -= timeMult;
				if (_levelReadyRetryTimeout <= 0.0f) {
					LOGD("[MP] Still not synchronized, re-reporting LevelReady");
					SendLevelReady();
				}
			}
		}
		...
		if (_updateTimeLeft <= 0.0f) {
			...
			switch (_levelState) {
				...
				case LevelState::PreGame: {
					if (_isServer && _gameTimeLeft <= 0.0f) {
						_waitingForPlayerCount = GetWaitingForPlayerCount();
						if (_waitingForPlayerCount <= 0) {
							StartCountdown();
						} else {
							_levelState = LevelState::WaitingForMinPlayers;
							// A peer that never finishes joining must not deadlock the round forever
							_waitingForReadyTimeLeft = MaxWaitingForReadySecs * FrameTimer::FramesPerSecond;
						}
				...
				case LevelState::WaitingForMinPlayers: {
					if (_isServer) {
						// Hold the round until everyone who is actually here can play, so a slow console (a real
						// PSP, several seconds behind) is not left at the gate -- in Race that is decisive.
						// Recomputed every tick, so someone joining or dropping changes the answer immediately.
						if (_waitingForReadyTimeLeft > 0.0f) {
							_waitingForReadyTimeLeft -= timeMult;
						}

						const std::int32_t waiting = GetWaitingForPlayerCount();
						if (waiting != _waitingForPlayerCount) {
							_waitingForPlayerCount = waiting;
							SendLevelStateToAllPlayers();
						}

						// The timeout is a backstop for a peer that never finishes joining, NOT a licence to start
						// a match nobody can play: it can override "someone is still loading", but never
						// MinPlayerCount, so a lone player still waits for an opponent instead of starting alone.
						const bool enoughPlayers = (GetNonSpectatePlayerCount() >= MinPlayersToPlay());
						if (waiting <= 0 || (enoughPlayers && _waitingForReadyTimeLeft <= 0.0f)) {
							if (waiting > 0) {
								LOGW("Starting round without {} player(s) that never finished joining", waiting);
							}
							StartCountdown();
							SendLevelStateToAllPlayers();
						}
				...
				case LevelState::Running: {
					if (_isServer) {
						// If a competitive round drops below the playable minimum (e.g. one of two players quit),
						// it is unwinnable - fall back to waiting instead of running a dead match. When enough
						// players return, WaitingForMinPlayers restarts the round with a fresh countdown.
						if (serverConfig.GameMode != MpGameMode::Cooperation &&
							GetNonSpectatePlayerCount() < MinPlayersToPlay()) {
							_levelState = LevelState::WaitingForMinPlayers;
							_waitingForReadyTimeLeft = MaxWaitingForReadySecs * FrameTimer::FramesPerSecond;
							_waitingForPlayerCount = GetWaitingForPlayerCount();
							SendLevelStateToAllPlayers();
			...
			if (_isServer) {
				if (_networkManager->HasInboundConnections()) {
					const TimeStamp packStarted = TimeStamp::now();
					const bool forceResync = _forceResyncPending;
					...
					const std::uint32_t serializeMicros = (std::uint32_t)(packStarted.microsecondsSince() + 0.5f);
					auto smoothMetric = [samples = _actorUpdateMetrics.Samples](std::uint32_t previous, std::uint32_t current) {
						return (samples == 0 ? current : (previous * 7 + current) / 8);
					};
					_actorUpdateMetrics.CandidateActors = smoothMetric(_actorUpdateMetrics.CandidateActors, actorCount);
					_actorUpdateMetrics.EmittedActors = smoothMetric(_actorUpdateMetrics.EmittedActors, actorCount);
					_actorUpdateMetrics.PacketBytes = smoothMetric(_actorUpdateMetrics.PacketBytes, (std::uint32_t)packet.GetSize());
					_actorUpdateMetrics.SerializeMicros = smoothMetric(_actorUpdateMetrics.SerializeMicros, serializeMicros);
					_actorUpdateMetrics.Samples++;
					_actorUpdateMetrics.ForceResync = forceResync;
					...
					_networkManager->SendTo([this](const Peer& peer) {
						auto peerDesc = _networkManager->GetPeerDescriptor(peer);
						return (peerDesc && peerDesc->LevelState >= PeerLevelState::LevelSynchronized);
					}, _forceResyncPending ? NetworkChannel::Main : NetworkChannel::UnreliableUpdates, (std::uint8_t)ServerPacketType::UpdateAllActors, packet);
				...
				if (!_players.empty()) {
					...
					// Deliberately unreliable: this is a complete snapshot re-sent every tick, so a lost one is
				// superseded ~33ms later by newer truth. Promoting it to the reliable stream would be worse, not
				// safer - PTP would block every newer position behind the retransmission of an already-obsolete
				// one, turning a loss that currently self-corrects into a visible stall. (Contrast
				// PlayerKeyPress, which is reliable precisely because nothing re-states it.)
	...
	void MpLevelHandler::HandleLevelChange(LevelInitialization&& levelInit)
	{
		...
		NotifyPeersOfLevelLoad(levelInit.LevelName, levelInit.LastExitType);
		LevelHandler::HandleLevelChange(std::move(levelInit));
	}

	void MpLevelHandler::NotifyPeersOfLevelLoad(StringView levelName, ExitType exitType)
	{
		if (!_isServer || levelName.empty()) return;
		constexpr std::uint8_t TransitionPacketVersion = 1;
		const std::uint16_t levelNameLength = (std::uint16_t)std::min<std::size_t>(levelName.size(), UINT16_MAX);
		MemoryStream packet(4 + levelNameLength);
		packet.WriteValue<std::uint8_t>(TransitionPacketVersion);
		packet.WriteValue<std::uint8_t>((std::uint8_t)exitType);
		packet.WriteValueAsLE<std::uint16_t>(levelNameLength);
		packet.Write(levelName.data(), levelNameLength);
	...
	bool MpLevelHandler::ShouldSerializePlayer(const Actors::Player* player) const
	{
		if (IsLocalSession()) {
			return true;
		}
		auto* mpPlayer = static_cast<const Actors::Multiplayer::MpPlayer*>(player);
		auto peerDesc = mpPlayer->GetPeerDescriptor();
		return (peerDesc != nullptr && !peerDesc->RemotePeer);
	}
		...
		if (_isServer) {
			if (auto peerDesc = _networkManager->GetPeerDescriptor(peer)) {
				...
				if (MpPlayer* player = peerDesc->Player) {
					...
					peerDesc->CarryOverHealth = player->_health;
					...
					const auto& serverConfig = _networkManager->GetServerConfiguration();
					// Nothing to do here for LevelState::WaitingForMinPlayers: _waitingForPlayerCount is derived
					// from live state every tick, so this departure is picked up (and broadcast) on its own.
					if (_levelState == LevelState::Running && (serverConfig.GameMode == MpGameMode::TreasureHunt || serverConfig.GameMode == MpGameMode::TeamTreasureHunt)) {
	...
	bool MpLevelHandler::CapturePeerState(const Peer& peer, PlayerCarryOver& carryOver, std::int32_t& health) const
	{
		if (!_isServer) {
			return false;
		}
		...
		carryOver = peerDesc->Player->Player::PrepareLevelCarryOver();
		health = peerDesc->Player->_health;
		return true;
	}
	...
	bool MpLevelHandler::OnPacketReceived(const Peer& peer, std::uint8_t channelId, std::uint8_t packetType, ArrayView<const std::uint8_t> data)
	{
		...
		if (_isServer) {
			...
			switch ((ServerPacketType)packetType) {
				...
				case ServerPacketType::ClearRemoteActors: return HandleServerPacketClearRemoteActors(peer, data);
	...
	bool MpLevelHandler::HandleClientPacketLevelReady(const Peer& peer, ArrayView<const std::uint8_t> data)
	{
		...
		if (auto peerDesc = _networkManager->GetPeerDescriptor(peer)) {
			...
			// Deliberately no "does this peer need the character-select lobby?" decision here. This packet is
			// processed the moment it arrives, which is NOT the moment the peer's identity is known: the app
			// drains the network (main.cpp: networkManager_->Update()) one line BEFORE it fills in the peer's
			// name/character/carry-over from the ad-hoc lobby (ConfigureConnectedGuests()). A guest that loads
			// faster than the host has its LevelReady already buffered when AttachAdhoc mints the descriptor, so
			// PreferredPlayerType is still None here and the peer was wrongly diverted to character-select and
			// never spawned - while a slow console (a real PSP) reported a frame later and worked fine. The
			// decision belongs in SynchronizePeers, which re-evaluates it every tick against settled state.
		}
		return true;
	}
	...
	{
		if DEATH_UNLIKELY(data.size() < 6) {
			LOGW("[MP] ClientPacketType::PlayerReady [{}] - Invalid packet size", peer);
			return true;
		}
		...
		LOGI("[MP] ClientPacketType::PlayerReady [{}] - type: {}, team: {}, fur: 0x{:.8x}", peer, preferredPlayerType, preferredTeam, furColor);
	...
	bool MpLevelHandler::HandleClientPacketPlayerUpdate(const Peer& peer, ArrayView<const std::uint8_t> data)
	{
		...
		InvokeAsync([this, peer, playerIndex, now, posX, posY, speedX, speedY, flags]() mutable {
			...
			// A client only reports its player's position once that player actually exists on its side, so this
			// packet is itself proof the spawn landed - no separate confirmation needed. The carry-over is held
			// until that proof arrives: CreateControllablePlayer used to clear it the instant it was serialized,
			// so if the frame was refused the guest got no body AND its progression was destroyed on both sides
			// with nothing left to re-send.
			peerDesc->PlayerBodyConfirmed = true;
			if DEATH_UNLIKELY(peerDesc->HasCarryOver) {
				peerDesc->HasCarryOver = false;
				peerDesc->CarryOverHealth = -1;
			}
	...
	bool MpLevelHandler::HandleServerPacketLoadLevel(const Peer& peer, ArrayView<const std::uint8_t> data)
	{
		...
		if (data.size() < HeaderSize || data[0] != TransitionPacketVersion) {
			LOGW("[MP] Invalid ServerPacketType::LoadLevel header");
			return true;
		}

		const ExitType exitType = (ExitType)data[1];
		const std::uint16_t levelNameLength = (std::uint16_t)data[2] | ((std::uint16_t)data[3] << 8);
		if (levelNameLength == 0 || levelNameLength != data.size() - HeaderSize) {
			LOGW("[MP] Invalid ServerPacketType::LoadLevel payload length: {}", levelNameLength);
			return true;
		}

		LevelInitialization levelInit;
		PrepareNextLevelInitialization(levelInit);
		levelInit.LevelName = String(NoInit, levelNameLength);
		std::memcpy(levelInit.LevelName.data(), data.data() + HeaderSize, levelNameLength);
		levelInit.LastExitType = exitType;
		levelInit.IsLocalSession = false;

		// Start to ignore all incoming packets, because they no longer belong to this handler
		_ignorePackets = true;

		LOGD("[MP] ServerPacketType::LoadLevel - level: \"{}\", exitType: {}", levelInit.LevelName, exitType);
	...
	bool MpLevelHandler::HandleServerPacketCreateRemoteActor(const Peer& peer, ArrayView<const std::uint8_t> data)
	{
		...
		std::uint32_t paletteColor = ((flags & 0x02) != 0 ? packet.ReadValueAsLE<std::uint32_t>() : 0);
		...
		InvokeAsync([this, actorId, flags, posX, posY, posZ, state, metadataPath = std::move(metadataPath), anim, rotation, scaleX, scaleY, rendererType, paletteColor]() {
			{
				std::unique_lock lock(_lock);
			...
			// Must happen before AssignMetadata so the sprites load indexed and the palette is applied
			if (paletteColor != 0) {
				remoteActor->SetPlayerColor(paletteColor);
			}
	...
	bool MpLevelHandler::HandleServerPacketClearRemoteActors(const Peer& peer, ArrayView<const std::uint8_t> data)
	{
		LOGD("[MP] ServerPacketType::ClearRemoteActors");

		InvokeAsync([this]() {
			std::unique_lock lock(_lock);

			// The server is about to resend the full actor list, so drop everything left from before. Anything
			// still here is stale by definition; keeping it is what produces duplicate actors once the server
			// reissues ids (see the send site in SynchronizePeers). The local controllable player is not in
			// _remoteActors, so it is untouched.
			for (auto& [actorId, actor] : _remoteActors) {
				if (actor == nullptr) {
					continue;
				}
				if (!_players.empty()) {
					_players[0]->CancelCarryingObject(actor.get());
				}
				actor->SetState(Actors::ActorState::IsDestroyed, true);
			}

			_remoteActors.clear();
			_playerNames.clear();
			_positionsInRound.clear();
		});
		return true;
	}
	...
	bool MpLevelHandler::HandleServerPacketPlayerWarpIn(const Peer& peer, ArrayView<const std::uint8_t> data)
	{
		...
		InvokeAsync([this, exitType]() {
			if (GetGameMode() == MpGameMode::Cooperation) {
				_remoteLevelExitType = exitType;
			}
			if (!_players.empty()) {
				auto* player = static_cast<RemotablePlayer*>(_players[0]);
		...
		switch (type) {
			...
			case AssetType::TileSet: {
				...
				if (!forWrite && !fs::IsReadableFile(fullPath)) {
					...
					if (!fs::IsReadableFile(fullPath)) {
						fullPath = fs::CombinePath({ resolver.GetCachePath(), "Tilesets"_s, String(path + ".j2tpsp"_s) });
						if (!fs::IsReadableFile(fullPath)) {
							fullPath = {};
						}
					}
	...
	std::shared_ptr<Actors::Player> MpLevelHandler::CreateResumablePlayer(std::int32_t index)
	{
		...
		peerDesc->PlayerName = PreferencesCache::GetEffectivePlayerName();
		peerDesc->FurColor = PreferencesCache::PlayerFurColor;
		auto player = std::make_shared<Actors::Multiplayer::LocalPlayerOnServer>(peerDesc);
		peerDesc->Player = player.get();
		return player;
	}
		...
		for (auto& [peer, peerDesc] : *_networkManager->GetPeers()) {
			...
			if (peerDesc->LevelState == PeerLevelState::LevelLoaded) {
				if DEATH_LIKELY(peerDesc != nullptr && peerDesc->PreferredPlayerType != PlayerType::None) {
					...
					// A peer with no character has not been through the ad-hoc lobby, so let it pick one here.
					// This runs on the same tick that decides PlayerReady vs LevelSynchronized, against state the
					// app has already settled for this frame - unlike the LevelReady packet edge, which can fire
					// before the peer's character is known and strand a fast guest in character-select forever.
					auto& serverConfig = _networkManager->GetServerConfiguration();

					std::uint8_t lobbyFlags = 0x01 | 0x02 | 0x04; // Set Visibility | Show | SetWelcomeMessage

					MemoryStream lobbyPacket(6 + serverConfig.WelcomeMessage.size());
					lobbyPacket.WriteValue<std::uint8_t>(lobbyFlags);
					lobbyPacket.WriteValue<std::uint8_t>(serverConfig.AllowedPlayerTypes);
					lobbyPacket.WriteVariableUint32(serverConfig.WelcomeMessage.size());
					lobbyPacket.Write(serverConfig.WelcomeMessage.data(), serverConfig.WelcomeMessage.size());

					_networkManager->SendTo(peer, NetworkChannel::Main, (std::uint8_t)ServerPacketType::ShowInGameLobby, lobbyPacket);
				}

				LOGI("Syncing peer [{}]", peer);

				// Wipe whatever the peer still has BEFORE anything authoritative is sent below, so nothing that
				// follows can be clobbered by it (this used to sit further down and wiped the positions that
				// CalculatePositionInRound had just populated). By this point the peer's actor map *should* be
				// empty - a fresh level handler starts empty - but nothing guarantees it: the peer only clears
				// its maps by reloading, and that reload hinges on a single unacknowledged LoadLevel packet.
				// Worse, actor ids are not stable across a reload (FindFreePlayerId restarts from 1 in
				// level-ready order), and the client's only duplicate guard is exact-id, so a stale map plus
				// reshuffled ids means every id that fails to collide creates a *second*, server-less actor -
				// the doubled, unshootable bodies seen after a round ends.
				_networkManager->SendTo(peer, NetworkChannel::Main, (std::uint8_t)ServerPacketType::ClearRemoteActors, {});
				...
				// In a competitive round already in progress, a hot-joiner watches as a spectator instead of
				// dropping into a match that is partway decided. The next round is a full level reload, which
				// rebuilds this peer's descriptor with no spectate flag, so they come back as a normal player
				// automatically. Cooperation still lets you jump straight in.
				if (_levelState == LevelState::Running && serverConfig.GameMode != MpGameMode::Cooperation) {
					canSpawn = false;
				}
				...
				if DEATH_LIKELY(canSpawn) {
					...
					// Keep an ad-hoc joiner clear of the host at the shared co-op start/checkpoint. Starting both players at
					// the exact same coordinates makes the authoritative player-collision resolver fight both clients.
					if (serverConfig.GameMode == MpGameMode::Cooperation) {
						spawnPosition.X += 30.0f;
					}
					...
					player->_health = (peerDesc->HasCarryOver && peerDesc->CarryOverHealth >= 0
						? peerDesc->CarryOverHealth
						: (serverConfig.InitialPlayerHealth > 0
						? serverConfig.InitialPlayerHealth
						: (PlayerShouldHaveUnlimitedHealth(serverConfig.GameMode) ? INT32_MAX : 5)));
					...
					// Spawn the player also on the remote side
					{
						...
						// MpPlayer::OnActivatedAsync (after the base reset). The flag is NOT cleared here: it is
						// cleared once the client reports the player back (HandleClientPacketPlayerUpdate), which is
						// the only evidence the spawn actually landed. Clearing it on serialization meant a refused
						// frame destroyed the progression with nothing left to re-send it from.
					...
					// _waitingForPlayerCount is derived every tick in the WaitingForMinPlayers state, which picks
					// this spawn up on its own and broadcasts only when the number actually changes.
				} else {
	...
	// Server: the smallest number of non-spectating players a FINISHABLE match needs. Competitive modes need at
	// least two (a one-player deathmatch/race/CTF cannot be won), Cooperation is fine solo. Never below the
	// configured MinPlayerCount.
	std::int32_t MpLevelHandler::MinPlayersToPlay() const
	{
		if (_networkManager == nullptr) return 1;
		auto& serverConfig = _networkManager->GetServerConfiguration();
		const std::int32_t configured = (std::int32_t)serverConfig.MinPlayerCount;
		return std::max(configured, serverConfig.GameMode == MpGameMode::Cooperation ? 1 : 2);
	}

	// Server: how many more players the round is still waiting for. DERIVED from live state on every tick rather
	// than latched at the moment something happens, so a late joiner or a disconnect is accounted for immediately
	// without any call site having to remember to update it - that latching is exactly why the number used to sit
	// at a stale 0 while the round waited on something else entirely.
	//
	// There are two independent reasons to wait, and the larger one is what we report:
	//   - too few combatants to satisfy MinPlayerCount ("we need more people")
	//   - everyone is here, but somebody has not finished joining ("we need to wait for them")
	// Both read naturally as "Waiting for N more players", which is what the HUD prints.
	std::int32_t MpLevelHandler::GetWaitingForPlayerCount()
	{
		if (!_isServer || _networkManager == nullptr) {
			return 0;
		}

		auto& serverConfig = _networkManager->GetServerConfiguration();
		const std::int32_t shortfall = std::max(MinPlayersToPlay() - GetNonSpectatePlayerCount(), 0);

		// How many guests are in this session at all. Taken from the ad-hoc roster rather than from GetPeers(),
		// because a guest that is STILL LOADING has not completed the engine-transport barrier and therefore has
		// no PeerDescriptor yet - the game layer is blind to it during exactly the window we must wait through.
		// Counting only descriptors is why a round could start while a real PSP was still loading the level.
		const std::int32_t expectedGuests = (std::int32_t)_networkManager->GetConnectedGuestCount();

		// ... and how many of them we can account for, i.e. are either in and playing or not going to play.
		std::int32_t accountedGuests = 0;
		for (const auto& [peer, peerDesc] : *_networkManager->GetPeers()) {
			// The host is not a guest; it simulates its own player and has nothing to confirm
			if (peerDesc->Player != nullptr && IsLocalPlayer(peerDesc->Player)) {
				continue;
			}
			// Spectators are not combatants, and a peer still in character-select is not joining THIS round -
			// waiting on either would hang the round for as long as they sit there. Both are accounted for.
			if (peerDesc->IsSpectating != SpectateMode::None || peerDesc->PreferredPlayerType == PlayerType::None) {
				accountedGuests++;
				continue;
			}
			// Really in: the guest has reported its player back, which is proof it exists on its side.
			// Deliberately NOT keyed on PeerLevelState, which only records what the server *sent*, and whose
			// Spectating rung nothing ever assigns.
			if (peerDesc->Player != nullptr && peerDesc->PlayerBodyConfirmed) {
				accountedGuests++;
			}
		}

		const std::int32_t stillJoining = std::max(expectedGuests - accountedGuests, 0);
		return std::max(shortfall, stillJoining);
	}

	void MpLevelHandler::StartCountdown()
	{
		_levelState = LevelState::Countdown3;
		_gameTimeLeft = FrameTimer::FramesPerSecond;
		SetControllableToAllPlayers(false);
	...
	Vector2f MpLevelHandler::GetSpawnPoint(PlayerType playerType, std::uint8_t team)
	{
		if (!_multiplayerSpawnPoints.empty()) {
			// Hand out consecutive spawn points instead of sampling with replacement: everyone spawning at the
			// start of a round drew independently, so two players could land on the same point even when the
			// level had plenty. The cursor persists across calls, so consecutive spawns are distinct as long as
			// the level provides enough points.
			...
			if (IsTeamGameMode(serverConfig.GameMode)) {
				...
				if (!candidates.empty()) {
					return _multiplayerSpawnPoints[candidates[_nextSpawnPointIndex++ % candidates.size()]].Pos;
				}
			}
			return _multiplayerSpawnPoints[_nextSpawnPointIndex++ % _multiplayerSpawnPoints.size()].Pos;
		} else {
			// The level has no multiplayer spawn points at all (some stock Battle maps ship without them), so every
			// player falls back to the single-player start for their character -- which stacks them on one tile.
			// Spread them horizontally so they at least don't spawn inside each other; the level still needs real
			// LevelStartMultiplayer events to be properly playable.
			...
			if (spawnPosition.X >= 0.0f && spawnPosition.Y >= 0.0f) {
				constexpr float FallbackSpreadPx = 24.0f;
				const std::uint32_t slot = _nextSpawnPointIndex++;
				// 0, +24, -24, +48, -48, ...
				const float step = float((slot + 1) / 2) * FallbackSpreadPx;
				spawnPosition.X += ((slot & 1u) != 0 ? step : -step);
			}
	...
	void MpLevelHandler::InitializeCreateRemoteActorPacket(MemoryStream& packet, std::uint32_t actorId, const Actors::ActorBase* actor)
	{
		...
		// A recolored actor reaches the client as a plain RemoteActor, which loads the metadata's native palette
		// and has no way to know it should be tinted. Players get their color from MarkRemoteActorAsPlayer, but a
		// corpse is never marked as a player - so without this, every corpse showed up uncolored on the guests.
		// Virtual, not runtime_cast: this used to be `runtime_cast<const PlayerCorpse>`, the codebase's only
		// const runtime_cast, and on the -fno-rtti PSP build that returned nothing - so paletteColor stayed 0,
		// the colour flag was never set, and guests rendered every corpse in the default palette while the host
		// (which owns the real PlayerCorpse) looked right. A plain virtual has none of that fragility.
		std::uint32_t paletteColor = actor->GetReplicatedFurColor();

		std::uint8_t flags = 0;
		if (paletteColor != 0) {
			flags |= 0x02;	// 0x01/0x02/0x80 are free here; 0x04..0x40 are consumed by RemoteActor::SyncMiscWithServer
		}
		...
		// Appended last so the packet stays byte-compatible for every actor that has no palette of its own
		if (paletteColor != 0) {
			packet.WriteValueAsLE<std::uint32_t>(paletteColor);
		}
	}
