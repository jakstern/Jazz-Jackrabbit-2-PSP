...
namespace Jazz2::Multiplayer
{
	...
	*/
	...
	struct PeerDescriptor : public MpPlayerState
	{
		...
		bool HasCarryOver;
		/** @brief Health to restore with @ref CarryOver; negative means use the game-mode default */
		std::int32_t CarryOverHealth;
		/**
			@brief Whether this peer has confirmed, by reporting its player, that its body actually exists

			Server-side and inferred rather than announced: a client cannot send a position for a player it does
			not have, so its first update is the proof. @ref PeerLevelState::PlayerSpawned only records that the
			server *sent* a spawn, which is not the same thing.
		*/
		bool PlayerBodyConfirmed;
