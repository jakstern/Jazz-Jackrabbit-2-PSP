...
namespace Jazz2::Multiplayer
{
		...
		bool& outCheckpointsOrdered)
	{
		...
		// A real level grid is modest. Reject a degenerate or absurd size early: this also gives the optimizer an
		// upper bound on every per-tile allocation below, so sizing the int32 arrays can't overflow 32-bit size_t
		// on PSP (which is what -Walloc-size-larger-than flags on the unbounded make_unique<int32_t[]> otherwise).
		if (totalTiles < 1 || totalTiles > (1 << 24)) {
			LOGW("Cannot auto-place race checkpoints: unexpected level grid size ({}x{})", W, H);
			return;
		}
